...
internal sealed class InMemoryTodoListStore : ITodoListStore
{
    ...
    public Task SetTodayLineAsync(string workspaceId, string? anchorItemId, CancellationToken ct = default)
    {
        _todayLineByWorkspace[workspaceId] = anchorItemId;
        return Task.CompletedTask;
    }

    public Task<string?> GetTodayLineAnchorAsync(string workspaceId, CancellationToken ct = default) =>
        Task.FromResult(_todayLineByWorkspace.GetValueOrDefault(workspaceId));
