...
public sealed class TodoCompleteJourney(BrowserFixture browser) : IAsyncLifetime
{
    ...
    [Fact]
    public async Task Completing_todo_from_home_removes_it_and_reflects_in_note()
    {
        var desc = $"TDC-{Guid.NewGuid():N}"[..25];
        ...
        await _app.AssertTodoItemVisibleAsync(desc);
        await _app.CompleteTodoItemAsync(desc);
        await _app.AssertTodoItemGoneAsync(desc);

        await _app.ClickNoteInListAsync(title);
        await _app.AssertActionItemCompletedAsync(desc);
    }
}
