...
- **Command handlers own orchestration.** Each aggregate gets a `*CommandHandler` in `src/Api/CommandHandlers/`. The handler loads the stream, rebuilds the aggregate, executes the command, persists events, then updates the affected read-model projections **inline** (synchronously, same request) before returning — see `NoteCommandHandler.UpdateProjectionAsync`. There is no event dispatcher; projections are written by the handler that appended the events. A new projection is wired inline in its owning command handler(s) plus a rebuild path (`ProjectionRebuildHandler`), never as a separate event-handler class. API endpoints do HTTP only — parse request, call handler, return result. Never write `store.ReadAsync` or `store.AppendAsync` inside an endpoint lambda. (The async-projector split in [ADR 0009](docs/adr/0009-split-lambdas-cqrs-async-projectors.md) will later move projection-building onto DynamoDB Streams; until then, inline-in-handler is the pattern.)
