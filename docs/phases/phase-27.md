...
| 27-RYW | **Read-your-writes foundation (NEW — the real prerequisite the cutover exposed).** A command returns the stream position it wrote; a read can request "consistent as of position N" and the query side waits/polls the projection until it reaches N. Gives the client read-after-write without per-mutation optimism or magic timers — the proper CQRS answer (= Cosmos session / Mongo causal / Postgres `WAIT FOR LSN`). **Sliced vertically** — RYW-1 proves the whole loop on one call (add-a-to-do, read-your-writes against the real async projector), then RYW-2/3/4 scale the pattern flow-by-flow until the cutover is complete (no big-bang). **Specced:** [phase-27-ryw.md](phase-27-ryw.md). | Not Started | 27-B |
