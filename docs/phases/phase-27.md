...
| 27-B | **DynamoDB Stream + Projector Lambda, in shadow.** Enable a stream on `notetaker-events`; add a Projector Lambda (event-source mapping, DLQ, bisect-on-error, max-retry/record-age) that drives the 27-A `ProjectionUpdater` from stream records. **Inline updates stay** — the projector is redundant-but-idempotent, so reads are unchanged. Banks Streams/idempotency/DLQ/replay + observability with **zero** consistency change. | Done _(projector re-enabled in RYW-1 — now the sole read-model writer)_ | 27-A |
| 27-C | **Cut over to async (remove inline updates).** ⚠️ **Attempted (PR #250) + frontend follow-up (#251), then REVERTED.** The backend cutover + projector were correct, but the frontend was built for immediate consistency — it reads server truth after navigation and reconciles via refetch, which under async raced the lagging projector → read-after-write broke across navigation, and patching it reactively became whack-a-mole (multiple distinct races across `keys.noteCards`/`keys.note`/`keys.actions`, plus a regression). Reverted to inline immediate-consistency, then **completed incrementally via 27-RYW** (RYW-1…RYW-4) — async is live and the projector is the sole writer, so the cutover this slice could not land is now done. | Done — via 27-RYW | 27-RYW |
| 27-RYW | **Read-your-writes foundation (NEW — the real prerequisite the cutover exposed).** A command returns the stream position it wrote; a read can request "consistent as of position N" and the query side waits/polls the projection until it reaches N. Gives the client read-after-write without per-mutation optimism or magic timers — the proper CQRS answer (= Cosmos session / Mongo causal / Postgres `WAIT FOR LSN`). **Sliced vertically** — RYW-1 proves the whole loop on one call (add-a-to-do, read-your-writes against the real async projector), then RYW-2/3/4 scale the pattern flow-by-flow until the cutover is complete (no big-bang). **Done (core):** RYW-1…RYW-4 shipped — every handler append-only, projector sole writer, read-after-write via the `ConsistencyGate`; only optional RYW-D (SSE poke) remains. See [phase-27-ryw.md](phase-27-ryw.md). | Done | 27-B |
| 27-D | **Split the HTTP Lambda into Command + Query functions.** Two functions behind one HTTP API, split by **API Gateway method routing + per-function IAM** (same `Api` binary, no code split). Command ($LATEST, no SnapStart): writes + the two side-service GETs (calendar, transcribe creds) + admin rebuild; IAM = event store R/W + side services + draft + (rebuild-only) projection R/W. Query (SnapStart + `live` alias): all other GETs; IAM = projection tables **read-only** + proj-position read + draft `GetItem` + events `DescribeTable` (health) — zero event-store data access, zero write verbs. SnapStart on Query only → deploy-time neutral. Completes the 3-Lambda Stage-1 shape. **Live in prod (PR #278; shipped via deploy #574 — its own #569 flaked on the quarantined Tags E2E gate, then rode the next backend deploy). Prod confirmed: `CommandFunction` + `QueryFunction` + `ProjectorFunction`.** | Done | 27-RYW |
