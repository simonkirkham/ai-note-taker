...
| 27-B | **DynamoDB Stream + Projector Lambda, in shadow.** Enable a stream on `notetaker-events`; add a Projector Lambda (event-source mapping, DLQ, bisect-on-error, max-retry/record-age) that drives the 27-A `ProjectionUpdater` from stream records. **Inline updates stay** — the projector is redundant-but-idempotent, so reads are unchanged. Banks Streams/idempotency/DLQ/replay + observability with **zero** consistency change. | Done | 27-A |
