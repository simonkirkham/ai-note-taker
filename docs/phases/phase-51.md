...
| A failed automatic write-up could never be retried and its error message vanished on the next tab switch, leaving the meeting un-analysed with nothing saying the manual button needed pressing. The one-shot claim is handed back on failure — but only on the automatic path: releasing it after a failed *manual* retry wrote a meeting up a second time, over a summary that had already succeeded. | `RecordingWriteUpSurvivesTabSwitch.test.tsx` (2 more) |
