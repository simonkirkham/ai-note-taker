...
| A failed automatic write-up could never be retried and its error message vanished on the next tab switch, leaving the meeting un-analysed with nothing saying the manual button needed pressing. The claim is handed back on failure — and re-taken by any success, which is the half the first attempt missed: the app's own remedy path (write-up fails → press Analyse → it works → glance at another note) then wrote the meeting up a third time, over the summary just produced. | `RecordingWriteUpSurvivesTabSwitch.test.tsx` (4 more) |
