...
| 24-C | **Operability: per-projection summary, fault visibility, concurrency guard.** Return a per-projection count map (not one note count); structured logs + EMF metric + alarm on rebuild faults/duration; reject overlapping rebuilds. | Done | 24-A |
