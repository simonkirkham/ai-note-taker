...
**Owner bring-online — DONE (2026-06-25): the secured connector is live.** Cowork connects via Google OAuth and lists notes. Config used: `MCP_OAUTH_CLIENT_ID` (secret) + `MCP_OAUTH_ISSUER` (variable) set on the **GitHub `Production` environment** (NOT repo-level — the issuer must match each env's `execute-api` host, and Test is a different account); applied to prod by re-running the backend deploy (#643) so `cdk deploy` re-read them. The Google `/oauth/google/callback` redirect URI was registered on the Phase 8 OAuth client (`175601380067-…apps.googleusercontent.com`) — a `redirect_uri_mismatch` on first attempt was that registration missing; it defaults to `{issuer}/oauth/google/callback` (`MCP_OAUTH_GOOGLE_REDIRECT_URI` unset). Prod verified: `/oauth/authorize` 400 (configured, was 503), MCP endpoint 401, AS metadata 200.

**Phase 35 complete (2026-06-25).** The read tools originally scoped as 35-B `get_note` / 35-C `search_notes` / 35-D `get_action_items` were **absorbed into 35-F** and shipped there (workspace-parameterized on the single `/mcp` connector, behind OAuth) — see the 35-F status log above. No standalone B/C/D slices remain.
