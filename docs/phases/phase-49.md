...
- **Recording guard:** a tab click while `NoteView` is recording must route through the existing leave-confirm, not a bare `navigate`. Reuse the BUG-34 machinery rather than adding a second confirm path — `NoteView` already traps `popstate` and shows `confirmingLeave` (`NoteView.tsx:263`). Simplest correct wiring: make the tab click a normal in-app navigation so the existing trap fires; verify by test, and if the trap does not cover it, lift the guard into a callback `NoteView` registers. **Breaker: write this scenario first — it is the slice's only real risk.**
...
- **A11y:** `role="tablist"` / `role="tab"` on the bar, `aria-selected` on the active tab, arrow-key movement between tabs, close button is a real `<button>` with `aria-label="Close <title>"` (same bar the `FolderPreviewPanel` keyboard fix set — never a clickable `<div>`).
- **Tests:**
