...
- **26-C lengthens every deploy** by the bake window — accept a short linear/canary config; this is a resilience/learning slice, not a current-downtime fix.
  - **Chosen config: `CANARY_10PERCENT_5MINUTES`** (10% for 5 min, then 100%). `cdk deploy` blocks on the bake because the CodeDeploy-managed alias update only completes when the shift does, so **every main deploy gains ~5 min** (whether or not the Lambda changed — SnapStart publishes a new version each code change). The `Deploy infrastructure` step is capped at `timeout-minutes: 25` so a stuck/rolling-back deployment fails the job fast instead of hanging.
  - **Idle-deploy caveat:** an alarm needs traffic to evaluate. On this single-user app most deploys have no concurrent traffic during the bake, so the canary usually completes with nothing to trip — the rollback protection is real only when a deploy coincides with live use. Accepted: the cost is bounded (5 min) and the mechanism still guards the one case that matters (a bad version under load).
  - A tripped alarm makes CodeDeploy roll back, which **fails the `cdk deploy` step** (red deploy job) — intended: a rollback is an event that needs a human to look.
