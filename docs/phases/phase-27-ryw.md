...
| RYW-3b | **Scale: folder + workspace flows.** Migrate folder create/rename/move/delete and workspace ops to async + RYW; their reads (`GET /folders`, `GET /workspaces`) gate on the token; `folder-`/`workspace-` join `MigratedPrefixes`. (The `FolderDeleted`/`WorkspaceDeleted` arms in `ProjectionUpdater` + `StreamProjector` routing are **already present** — re-added in RYW-1's foundation — so the running projector already deletes, not re-creates; 3b just removes the inline writes.) | In Progress | RYW-1 |
