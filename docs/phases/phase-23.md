...
|-------|---------|--------|------------|
| 23-A | **Workspace aggregate + list + CRUD API + default workspace.** `Workspace` aggregate (create/rename/delete), `WorkspaceList` projection + table, `/workspaces` endpoints, reserved non-deletable default "Personal". No content scoping yet. | Not Started | — |
