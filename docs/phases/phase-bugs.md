...
| BUG-25 | `ActionItemJourney.Action_items_persist_across_navigation` E2E flakes on the deploy gate — RYW-3a made the actions read (`GET /notes/{id}/actions`) async + token-gated, but this **pre-existing** journey asserts the action visible after reopening the note with a plain 5 s locator wait (no reload-tolerance), so under the deployed projector's cold-start lag (> the gate's ~2.6 s bound) it serves `stale` and the assert times out. Same class as the cards (#256) / tags (#265) hardening that earlier async flows already needed; the **new** `ActionReadYourWritesJourney` was hardened, this one was missed. Observed on deploy #560. Fixed PR #275 (reload-tolerant assert), deploy #565 green (E2E 23/23). | Done | RYW-3a, BUG-26 |
| BUG-26 | Deploy E2E gate is intermittently red because **post-navigation visibility asserts race the async projector** — a *different* journey times out at the 5 s locator bound each run (#548 `NoteImageJourney`, #549 `TagsJourney`×2, #560 `ActionItemJourney`). Systemic consequence of the RYW async migration: any journey reading a projector-built view after a write+navigation depends on the deployed projector folding within 5 s, which the token gate only bounds to ~2.6 s. Reactively hardened per-journey so far (cards #256, tags #265); needs a systematic fix (reload-tolerant helper for every projector-backed post-nav assert, or a longer locator timeout for those reads). Reds block the "main deploy green" merge gate and bury real failures among flakes. *(Test-infra robustness — could alternatively live in `technical-improvements.md`.)* **Done 2026-06-13 via TI-39** — systematic fix (projector warm-up/drain + 15 s global timeout + reload-tolerant asserts **and actions**) cleared every journey; the two real bugs it masked ([BUG-27], [BUG-29]) were fixed and the residual concurrent-multi-tag race carved out as [BUG-28]. Write-up: [docs/learnings/deploy-gate-deflake-stacked-causes.md](../learnings/deploy-gate-deflake-stacked-causes.md). | Done | RYW-2 |

| BUG-27 | **Concurrent multi-tag add silently drops a tag — an exhausted-retry contention conflict is reported to the client as success.** A space-separated paste (`"1:1s Bill"`) fans into two concurrent `POST /notes/{id}/tags` on the **same** note stream. `NoteCommandHandler` retries the optimistic-concurrency conflict, but on exhaustion **rethrows `ConcurrencyException` → 409** — and the client treats **409 as a duplicate-tag no-op** (`tags.ts`), so the loser's `NoteTagged` is never appended yet no error fires: the optimistic pill stays over an unpersisted write, and every later projector re-fold correctly lacks the tag. Pre-existing; **aggravated by 27-D** (concurrent writes now land in separate Command-Lambda invocations, racing the DynamoDB transaction harder). Surfaced as the residual `TagsJourney.RemoveTag_*` deploy-gate flake under TI-39. **Fix:** server throws `WriteContentionException` on exhaustion → maps to a **retriable 503** (distinct from the duplicate 409), `MaxAppendAttempts` 4→6; client `tagNote`/`untagNote` retry on 503 (409/404 stay no-ops), throwing on the final attempt so a never-landing write rolls back instead of silently dropping. | Done | RYW-2, BUG-17, BUG-26 |

| BUG-28 | **Concurrent multi-tag add *then remove* leaves the surviving tag/card unreadable under the async projector.** After `AddTagAsync("1:1s Bill")` (two simultaneous `POST /notes/{id}/tags` on one note stream) then removing `"Bill"`, the surviving `"1:1s"` pill (`RemoveTag_PillDisappears`) and the note **card** (`RemoveTag_GoneAfterNavigation`) intermittently never appear — failing even a 30 s reload-tolerant, token-gated wait against a warmed projector. **Distinct from BUG-27** (whose contention-503 fix did *not* resolve this) and from the cold-projector flakes (TI-39 warm-up cleared every other journey). **Not reproducible in-process** — the synchronous `ConcurrencyConflict_OnSecondOfTwoTagAdds_BothPersist` spec passes — so it is a true-concurrency / distributed-projection phenomenon (the 27-D Command-Lambda split races two real DynamoDB transactions). **Evidence:** the most persistent failure today — `RemoveTag_GoneAfterNavigation` ×8 attempts, `RemoveTag_PillDisappears` ×7, the only tests still failing after both fixes (last #571). **Root cause (confirmed by deployed-env repro — 15/20 dropped, showing 500s + 404s, not 503/409):** the loser of two concurrent `TransactWriteItems` on the same stream's META row is cancelled by DynamoDB with reason **`TransactionConflict`**, not `ConditionalCheckFailed`. `DynamoDbEventStore.AppendAsync` only translated `ConditionalCheckFailed` → `ConcurrencyException`, so a `TransactionConflict` escaped uncaught → **unhandled 500**, and since the command handler only retries `ConcurrencyException`, the write was **silently dropped**. (Not reproducible in-process: the in-memory store has no transaction-conflict semantics; `ReadAsync` is `ConsistentRead`, ruling out stale reads.) **Fix:** treat `TransactionConflict` as a retriable conflict too (→ `ConcurrencyException` → handler re-reads + re-appends), classifier extracted + unit-tested; `ConcurrencyException.Reason` threaded into the `InstrumentedEventStore` conflict warning so prod can confirm the path. General — protects every aggregate's concurrent writes, not just tags. Two `RemoveTag_*` tests un-quarantined. | Done | BUG-27, RYW-2, 27-D |

| BUG-29 | **Projector can't purge note images on delete — role missing `s3:ListBucket`.** The Projector Lambda role was granted `imagesBucket.GrantDelete(…, "notes/*")` (DeleteObject only), but `S3NoteImageStore.PurgeNoteAsync` **lists** the note's objects before deleting them, so every `NoteDeleted` purge throws `AccessDenied (s3:ListBucket)` — caught at Warning (`ProjectionUpdater.cs:48`) but **orphaning all of a deleted note's images in S3** indefinitely. Found 2026-06-13 in the prod Projector log group while investigating BUG-28; a 27-RYW/27-D IAM-scoping gap. **Fix:** add `imagesBucket.GrantRead(projectorFunction, "notes/*")` (supplies `ListBucket`+`GetObject`) alongside the existing `GrantDelete`; CDK assertion updated to require a List action on the projector role (still no PutObject). | Done | 27-D |

| BUG-30 | **Note-scoped handlers authorize against the async NoteDetail projection → 404 after create under load.** Every note-scoped write (date/title/tags/content/delete/move/unfile, action add/complete/reopen/delete) and the actions read checked ownership via `noteDetailStore.GetAsync(noteId)` then 404'd on null. Since 27-RYW, `NoteDetail` is built **asynchronously** by the projector, so an operation firing shortly after `POST /notes` hits a not-yet-built projection → **404 "note not found"**, even though the note's event stream exists (writes use `ConsistentRead`). Under the E2E write burst the projector lags ~1–2 s → a 404 storm; in prod (single user, no burst) the projector keeps up so it was invisible — and invisible to investigation because **the E2E env is a separate AWS account** (no CloudWatch); only an `[api-fail]` Playwright response-logger surfaced the test-env 404s. The single largest residual deploy-gate flake (rotating journeys: AddTag/RemoveTag/NoteImage/Action). **Fix (multi-part):** (a) **writes** — `NoteCommandHandler` + a shared `INoteAuthorizer` authorize from the strongly-consistent **event stream** (owner = UserId on the note's first event; null = legacy), running in the Command Lambda (which has event-store access); write handlers fall through on a null projection to that authority. (b) **GetActions read** — runs in the **Query Lambda** (27-D: no event-store access, so the authorizer 500s there); kept on the projection but added a **bounded re-poll** to ride out a **cross-stream fold race** (the read gates on the *action* stream's position but ownership reads the *note* projection — DynamoDB Streams don't guarantee cross-key order, so the action can fold before the note → spurious 404). Cross-user isolation tests green throughout. | Done | 27-RYW, 27-D |

| BUG-31 | **`NoteImageJourney.Remove` — removed image still shows after reopen.** Investigated 2026-06-17 by un-quarantining under a hang-proof diagnostic (120 s per-test cap, PR #294) and rerunning on the real gate. It is **THREE stacked flake causes** (like TI-39 was four): **(1) image-reappears — the original symptom — ✅ FIXED** by the RYW systemic changes (TI-39 projector warm-up/drain + BUG-30 auth-from-event-stream); proven when attempts 1–2 passed the image-absent assert. **(2) ✅ FIXED (PR #297):** the shared `SaveAndReturnAsync` helper awaited a `GET /notes/cards` response React Query can serve **from cache → no request → 30 s timeout**; now waits on home navigation (`new-note-button`) instead — a suite-wide latent-flake removal, not just this test. **(3) 🔲 OPEN:** after fix #2, deploy #599 attempt 4 (~1/4) still failed at the post-removal save — `ClickAsync("save-button")` times out 30 s because the button is `disabled={loadingDetail}` (`NoteView.tsx:401`) and `useNoteDetail` stays `isLoading` ~30 s after reopen+edit (a **stuck note-detail read**, RYW/async-projection class). **Status:** re-quarantined (PR #298) with all three layers in the Skip reason; product behaviour (remove → stays gone) verified working. **Next step:** instrument the note-detail read to capture *why* `loadingDetail` hangs 30 s (cold/stuck gated GET, retry storm, or a 2nd `save-button` match) via the evidence-through-the-thrown-message pattern; then close layer 3 and fully un-quarantine. | Open | BUG-30, 27-RYW |
...
**Status:** ✅ Done — PR #273 (v2), deploy #563 (2026-06-13). **First attempt (PR #272) was merged then reverted** (commit `c1e3bb2`): it added a markdown-it renderer-rule to move the bare key out of `src`, passed a unit seam, but the deploy-gate E2E proved the 403 still fired — `tiptap-markdown`'s actual `content`/`setContent` parse bypassed the rule. **v2 (resolve-before-parse) is the real fix:** the editor is created with `content: stripImageKeys(value)` (bare-key images removed from the markdown string *before* any parse, so no `<img>` is built to fetch), then a once-per-note effect (`didResolve` ref + captured `initialValueRef`; editor remounts per `noteId`) calls `resolveImages` and `setContent(keysToSrcs(initial, urls), { emitUpdate:false })` so the only image src the parser ever sees is a presigned `…?X-Amz-…` URL (200). Insert/paste/drop are gated on a `canInsert` flag while resolving so the `setContent` can't wipe a user-inserted node. Save invariant (`srcsToKeys` → bare keys) preserved. **Proven by** the re-added `NoteImageJourney.Opening_a_note_with_an_image_issues_no_bare_key_relative_image_request` E2E (passed in a real browser at the deploy gate — total 23, the BUG-24 test green). Found 2026-06-13, immediately after [TI-37] made the 403 visible in RUM.

**Lesson (folded into `_minor-log`):** a no-network-fetch property is **unprovable in jsdom** — the unit seam only proves the transform/ordering; the deploy-gate E2E is the authoritative proof, and PR CI (no E2E) will green a broken fix. v1 shipped on unit-only confidence and cost a merge→revert→re-fix cycle.
...
**Status:** ✅ Done — PR #275, deploy #565 green (2026-06-13; E2E 23/23, the hardened journey passed against the live async projector). The fix was exactly the **fix direction** below: the two `AssertActionItemVisibleAsync` calls became reload-tolerant `AssertActionVisibleAfterReloadAsync`, and the now-unused plain `AssertActionItemVisibleAsync` page-object helper was removed (Hawk) so no future journey reaches for the flaky variant. Found 2026-06-13 reviewing the recent deploy E2E history; the actions read went async in RYW-3a (PR #263). The systemic pattern remains open as [BUG-26].
