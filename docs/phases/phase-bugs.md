...
| BUG-45 | A note moved to another workspace can reappear in the source workspace's list (and folder moves can flicker) — same ungated-cards-refetch defect class as [BUG-44], different trigger. `useMoveNoteToWorkspace`/`useMoveNoteToFolder` optimistically drop/patch the card then `onSettled → invalidate → getNoteCards`, but `moveNoteToWorkspace`/`moveNoteToFolder`/`unfileNote` use plain `requestVoid` (no token capture) and the `MoveNoteToWorkspace`/`MoveNoteToFolder`/`UnfileNote` handlers never call `SetConsistencyToken` → the refetch runs ungated against the async projector. Move-to-workspace = a note reappears where it was moved from (BUG-44 symptom); folder moves = a brief placement flicker (lower severity). Found by Hawk reviewing #382. Fix: emit + thread the token on all three, mirroring the BUG-44 delete fix. | Open | BUG-44, RYW-2 |
...
## BUG-45 — Move-to-workspace (and folder moves) not consistency-gated → moved note can reappear in the source list

**Status:** Open. **Severity:** Medium — move-to-workspace can reappear the note where it was moved *from* (BUG-44 symptom, no data loss); folder moves only flicker placement briefly. Found by Hawk reviewing #382.

**Root cause:** the exact ungated-cards-refetch defect [BUG-44] fixed, on the note-move paths that were not touched:
- `useMoveNoteToWorkspace` (`web/src/hooks/useNoteMutations.ts:82`) optimistically drops the card, then `onSettled → invalidate(noteCards) → getNoteCards`. But `moveNoteToWorkspace` (`web/src/api/folders.ts:~85`) uses plain `requestVoid` (no `captureNoteToken`) and the `MoveNoteToWorkspace` handler (`src/Api/Handlers/NoteHandlers.cs:~305`) never calls `SetConsistencyToken`. So the refetch runs against the not-yet-updated projector and the moved note reappears in the source workspace's cards.
- `useMoveNoteToFolder`/`unfileNote` (`web/src/hooks/useNoteMutations.ts:68`, handlers `NoteHandlers.cs:~287,~314`) have the same shape; the card changes `folderId` rather than disappearing, so worst case is a brief folder-placement flicker.

**Fix:** emit + thread the write token on all three, mirroring the BUG-44 delete fix — handler injects `HttpResponse` + `SetConsistencyToken(response, noteId, version)`; the api fn uses `requestVoidWithResponse` + `captureNoteToken`.

**Reproduce-before-fix:** `Api.Integration` — `POST /notes/{id}/workspace` (move) returns an `X-Consistency-Token`, and a token-gated `/notes/cards` for the source workspace excludes the moved note; frontend — after `moveNoteToWorkspace`, the cards refetch carries the move token in `If-Consistent-With`.
