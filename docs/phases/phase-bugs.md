...
**The same budget is also what caps ACCURACY, measured 2026-09-21.** A deliberate 30 s test spoken clearly into the mic came back as "Can you hear me? **It's** the transcription working. Please, can you **firm**? The transcription **worker is not**." — for "Is the transcription working? Please can you confirm the transcription is working." That text is **not** the degraded live preview: `diarizeStreams` runs `small.en` at full context (`localTranscription.ts:95,107` — no `--audio-ctx`), so it is already the best the on-device path produces. The next model up is **already on the user's disk** (`ggml-medium.en.bin`, 1.5 GB, downloaded 2026-07-24) and referenced by nothing: `modelStore.ts:29-31` records it was tried and rejected in [BUG-52] as "minutes of full CPU on a laptop (doubled for 1:1 diarization)". So accuracy and finalising time are one trade, not two problems — and the ~2× from a native ARM64 build, or the ~3.3× from Parakeet, is what would pay for a better model rather than just a faster wait.
...
**Fixed 2026-09-21 (awaiting deploy).** Four changes, all in the desktop shell:

| # | Change | Where |
| --- | --- | --- |
| 1 | A sustained run of failures against a ready engine now replaces it and resumes from the same `finalizedMs`, capped at 2 replacements per recording; past that the user is told once, as before | `streamingSession.ts` `recoverOrGiveUp` |
| 2 | The hung engine stops receiving requests at that point — the pile-up on whisper-server's serialising mutex is what made one stuck window permanent | same |
| 3 | A warm engine is health-checked (`probeAlive`) before reuse, so a corpse is never handed to the next recording | `whisperServer.ts`, `ensureServer` |
| 4 | A terminal live view stops stepping, instead of re-transcribing an impossible window for 7 m 23 s against the stop-time pass | `reportTerminal` |

**Two limits of the fix, both deliberate and both found in review rather than by reasoning:**

1. **Recovery needs a run of three consecutive timeouts.** The decision is taken from the error signature at the throw site, not by asking the engine whether it still answers — an earlier version did that and was self-defeating, because this bug's own mechanism is that the engine falls silent *as* abandoned requests accumulate, so at the decision point it is still answering and a real hang would have gone unreplaced, logging nothing to say so. The cost of the signature rule is that a mixed run — say every third failure a `500` — never reaches three consecutive timeouts and gives up instead. That is the intended answer for an engine that is erroring rather than jammed, since a replacement returns the same error.
2. **A single blip no longer ends the recording.** Initially "replace it" and "give up" shared one counter, and because that counter counted every cause while the timeout run counted only timeouts, one connection error among the first three failures killed recovery for the whole recording and stopped the step timer — a path straight back to this very bug, measured in review. The thresholds are now separate: replace on three consecutive timeouts, give up at six failures of any kind. The asymmetry is the reason — replacing needlessly costs one model load, failing to replace costs the rest of the meeting. **Six is the floor, not a round number:** the blip resets the timeout run, so the run only completes on the sixth failure; five would give up one step before it and reinstate the defect. **Accepted edge:** *two* blips still defeat recovery. That is the right stopping point — each further increment costs another 20 s of dead transcript in the case that matters — but it is the honest limit of the rule.
