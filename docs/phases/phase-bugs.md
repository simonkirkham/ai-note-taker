...
| BUG-31 | A browser test (remove an image, then reopen the note) randomly fails during deploys and blocks releases. Two stacked causes fixed. Re-enable attempt (PR #393) **flaked 1-pass/1-fail on deploy #697 and was reverted** (PR #395): the failure is the **post-removal save** (save-button missing/disabled), not a slow reopen, so reopen reload-tolerance can't fix it. Remaining: root-cause which note-view state removes the save button, fix in product code, then re-enable. Removing an image works correctly for real users. | In Progress | BUG-30, 27-RYW |
...
**Status:** In Progress — cause 3's **root cause is mitigated in prod** (RYW gate cap 2s→8s, PR #377, deploy #681) so a real user's cold-projector read is tolerated; two other causes already fixed. **Re-enable attempt failed (2026-07-17): reverted.** PR #393 re-enabled the test with a reload-tolerant *reopen* wait; deploy #697 flaked it **1 pass / 1 fail** across two attempts and it was reverted (PR #395, deploy #698). The failure is **not** a slow reopen — it is the **post-removal save** (`SaveAndReturnAsync` → `System.TimeoutException: waiting for GetByTestId("save-button")`, 30s, `NoteImageJourney.cs:202`). Reopen-side reload-tolerance **cannot** fix it: the hang is *after* the in-editor image removal, where a reload would drop the unsaved removal. So the save-button is missing/disabled at the post-removal save — a note-view state, not a wait-tuning problem. The feature itself works correctly for real users. **Remaining:** identify which note-view state removes the save button, fix it in product code, then re-enable.
