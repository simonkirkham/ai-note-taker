...
1. **Tell the user — slice 1, in progress.** After ~2 min with no new text the recording control says transcription has stopped, names which of the three cases it is, and says to stop and start recording again. This alone would have saved 3.5 h of the 2026-09-17 meeting.
...
**What slice 1 does NOT do.** Nothing reopens the stream and nothing refreshes credentials, so a recording that stops still has to be stopped and started by hand. Slice 1 makes the choice visible and gives the server the evidence to settle the leading candidate on the next occurrence; it does not prevent the loss.
