...
| BUG-31 | A browser test (remove an image, then reopen the note) randomly fails during deploys and blocks releases. Root-caused: the #697 flake was the note going **blank** at the post-removal save ([BUG-48]) which flips Save→Cancel by design. **Fixed + re-enabled (PR #397, deploy #701):** the test keeps durable body text so `hasContent` stays true and the Save button is present. Fix is **deterministic** (the stale-refetch race can no longer change the outcome). Now accruing deploy-gate confidence passes — **2/2 clean** (#701, #702), passively watching further natural deploys before flipping Done. Removing an image works correctly for real users. | In Progress | BUG-30, 27-RYW, BUG-48 |
...
**Status:** In Progress — **fixed + re-enabled (PR #397, deploy #701).** Accruing deploy-gate confidence passes before flipping Done: **2/2 clean** (#701 the merge, #702 44-B natural) — `Remove_an_image` ran and passed both (`Passed: 27, Skipped: 0`), no BUG-38 recurrence. Passive-watch decision (2026-07-23): the fix is **deterministic** (durable body text makes `contentDraft` non-null → `hasContent` always true after removal → the stale-refetch race cannot change the outcome), so no active reruns — accrue passes only from natural deploy traffic, flip Done once ~5 accrue.
