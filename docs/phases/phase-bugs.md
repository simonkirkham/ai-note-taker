...
**The self-check needed a self-check.** Its first 10-run batch was **6 passed, 4 failed** (E2E run [#179](https://github.com/simonkirkham/ai-note-taker/actions/runs/31544406237)), reporting that the probe could not tell a gated read from an ungated one. It could — the helper was sampling the wrong read. A reload produces **more than one** actions read: the note screen mounts and reads, carrying the persisted token, and because the server answers non-stale the client then CLEARS that token; opening the popover refetches, ungated. The helper reported the *last* read, so it reported the second, ungated refetch. It now reports the first read after the reload and samples before touching the popover. Worth keeping because it is the same shape as the bug itself — a measurement that looked like a property of the system was a property of when the measurement was taken.
