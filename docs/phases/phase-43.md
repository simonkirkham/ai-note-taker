...
_(43-F done — PR #428, deploy #724, `deploy-production` confirmed. **Its outstanding post-deploy rebuild was run on 2026-08-10** as step 2 of the 43-H1 rollout (189 `noteDetail` rows re-folded, 0 stale). Nested items DO count (flat). Two things the build added beyond the plan: `AgendaItemView.Derived` (a derived topic has no event stream, so the header shows it read-only until 43-G), and body-wins on a matched pair. ~~Outstanding: the post-deploy projection rebuild~~ — **run 2026-08-10** — measured 2026-08-07, 1 of 183 prod notes has task lines with no agenda.)_
...
- [x] **Applied 2026-08-10.** Rebuild first (189 noteDetail rows, 0 stale), then dry run, then apply: 8 notes / 36 topics, 0 stale, 0 failed, 0 excluded. The live dry run matched an independently-computed offline preview **byte-for-byte on all 8**. Verified against the event stream afterwards: each note's content is exactly the predicted value, the original body is preserved verbatim, paragraph count is +1 (the checklist) on every note, and the deleted 9th stream was untouched. A re-run now reports 0 notes with legacy topics — idempotent.
- [x] **Done 2026-08-10, after the migration was applied and verified.** Removed: the legacy fold and `_legacyAgenda` from `NoteDetailProjection`; `AgendaFromContent.Compose`/`MatchKey` (the union and its comparison key existed only for the strangler window, so `Composed` is now just `Parse`); the four write endpoints and their handlers; the four command records and their arms in `Note.Handle`; the aggregate's `_agendaItems`/`_agendaAddCount` state; the three frontend hooks, `useAgendaMutations.ts` and `api/agenda.ts` in full; and `AgendaMigrationHandler` with its endpoint, now dead because it read exactly the non-derived items that no longer exist. The events stay in the streams, unread — reversible.
- [x] **Carried from 43-G's review — NOT fixed in 43-H2; refiled as [BUG-76](phase-bugs.md) so they survive this phase closing.** They are agenda-count divergences, not part of the removal, and 43-H2 was already a 1,900-line deletion. Since 43-H2 the body is the only source, so these are now the only way the header and the note can disagree — the legacy union used to mask them. (a) The BUG-24 image-resolve calls `setContent` with `emitUpdate: false`, so the editor's live topic list is not republished for it; one extra `publish()` in that `.then` makes the live list unconditionally authoritative. (b) `collectTaskItems` recurses only into `taskList` children, so a task item reachable through a **non-checklist** list — `- Shopping` / `  - [ ] Milk`, or a bulleted child under a task item — is counted server-side but invisible in the header. Widening the recursion to a list-type set closes it and cannot reintroduce the blockquote exclusion (a blockquote is not a list). (c) A paragraph holding only a non-text inline node (`- [ ] ![shot](key)`) reads as empty client-side and is skipped, while the server counts it.
- [x] **No projection rebuild needed, deliberately.** This IS a fold change to a populated projection, which normally mandates one — but 43-H1's apply run proved the set it would affect is empty: 8 notes / 36 topics migrated, 0 stale, 0 excluded, and a re-run immediately after reported 0 notes with legacy topics. No stored row can still carry an unmatched legacy item, so re-folding would change nothing. (One real delta if a rebuild IS run: the removed arms used to bump `LastModifiedAt` on agenda events, so a note whose LAST event was an agenda event would get an earlier timestamp — post-migration every such note has a later `ContentEdited`, so that set is empty too.)
- [x] `EventDeserializer` keeps its `AgendaItem*` arms — **these are the load-bearing ones**: its default `throw`s on an unknown type, so removing them would fail every rebuild of the 9 affected streams. `Note.Apply` also keeps explicit no-op arms, but its default is already `break`, so those are documentary ("deliberately ignored", not "not yet handled"). Pinned by `A_stream_still_carrying_legacy_agenda_events_replays_and_ignores_them`, which drives the deserializer + projection path; `Note.Apply` is not exercised by it.
