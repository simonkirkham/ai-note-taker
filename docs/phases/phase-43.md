...
- [x] `EventDeserializer` keeps its `AgendaItem*` arms, and `Note.Apply` keeps explicit no-op arms for them, so history stays READABLE rather than merely un-thrown-on. Pinned by `A_stream_still_carrying_legacy_agenda_events_replays_and_ignores_them`.
