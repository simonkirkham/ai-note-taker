...
- **Acceptance criteria:**
  ...
  - [ ] The new field round-trips through `DynamoDbTodoListStore` (integration test). — **tests written, never executed.** `TodayLine_SurvivesTheRoundTrip`, `TodayLine_IsScopedPerUserNotJustPerWorkspace`, `TodayLine_ANullAnchorClearsTheStoredValue`, `TodayLine_MarkerRowIsNotReturnedAsAnItem` exist in `DynamoDbTodoListStoreTests`, but `EventStore.Integration` needs Docker and it was unavailable. This is the one criterion with **no** verification behind it, and per the `*View` guardrail it is the only suite that can catch a `DynamoDb*Store` mapping gap — the in-memory double keeps the value by reference and passes regardless.
- **Verification status (2026-08-06, pre-merge):** build clean; `Domain.Specs` 319/319; `Api.Integration` 701/701; frontend 860/860 across 86 files; `eslint` clean; `tsc` clean against **both** `tsconfig.json` and `tsconfig.test.json`. **Not run:** `EventStore.Integration` (no Docker) and the deploy-gate E2E. PR #423 is `MERGEABLE`/`CLEAN` but **no CI run exists for its head sha** — a GitHub Actions `major_outage` on 2026-08-06 meant the push event never created a run, and `pr.yml` has no `workflow_dispatch`, so it needs a fresh `pull_request` event (close/reopen) once runners return.
