...
- **Events/commands:** none new — reuses `ReorderTodos` plus 50-A's line anchor (`useReorderTodos` + `useSetTodayLine`). No new event, no CDK, no projection change.
- **Backend change (found in review, NOT optional):** `TodoOrderCommandHandler` had **no append retry**, unlike every other command handler. 50-B's "Move to Later" issues a `ReorderTodos` and a `SetTodayLine` in the same tick and **both append to the one stable-id stream** `todo-order#{workspaceId}` — so the pair races itself, and the loser's raw `ConcurrencyException` surfaced as a **409**, which the client treats as a duplicate/no-op and silently drops (BUG-27's class). Added the bounded read-rebuild-append retry mirroring `NoteCommandHandler` (6 attempts, exponential backoff + jitter), with `WriteContentionException` → **503** on exhaustion. This also closes the same latent race on 50-A's drag and `sendTo` paths, where it was an anchor-only edge case.
...
### Deploy-time
- **50-A:** Backend + frontend → full `cdk deploy`. New event type and a projection **field** — no new table, so no backfill and no new projection to populate. **Neutral**, no recurring per-deploy cost.
- **50-B:** scoped as frontend-only, but the review-found retry fix touches `src/Api/` → `detect-changes` reports **`backend=true`** and the deploy runs the full backend build + `cdk deploy`. This is the *wanted* outcome per the infra guardrail — a `backend=false` deploy would have shipped the frontend against the old handler and left the 409 race live in prod. No new resource, so no CDK update beyond the Lambda asset. Adds **one** deploy-gate journey (`TodayLineJourney`, two 30 s reload loops worst case). **Neutral to marginally slower** on the gate; no recurring per-deploy cost.
