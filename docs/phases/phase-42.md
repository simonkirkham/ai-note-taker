...
|-------|--------------------|--------|------------|
| 42-A | **`list_meetings(workspaceId, date)` + calendar resolution off the route.** Claude lists a workspace's meetings for a date (title, time, whether a note is linked). Proves the whole MCP→calendar pipe on one real read. | Not Started | — |
