...
| CHANGE-27 | Redesign the note-detail **Tags + Actions sidebar** to take less space — `TagsSection` + `ActionsSection` stacked in the fixed 320px column are too tall (each: heading + full-size pills/rows + an always-visible input). **Prototype several approaches first** (compact dense rows + small chips + inline add; collapsible accordion with count badges; tabbed single panel; inline tag-chip bar + actions-behind-a-summary toggle), pick one, then implement. Appearance/layout only — no change to tag/action capability. | Open | — |
| CHANGE-28 | Note **body text is too big** — the editor `.contentInput` renders at 16px (1rem) / line-height 1.75 on a 16px base. **Prototype** a smaller size, tighter line-height, and alternate font(s) to find a comfortable density, then apply. Appearance only (`NoteEditor.module.css` / token). | Open | — |

Open: CHANGE-27 (Tags+Actions sidebar redesign — needs prototype), CHANGE-28 (notes body text size/font — needs prototype).
