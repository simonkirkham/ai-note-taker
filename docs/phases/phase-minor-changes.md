...
| CHANGE-29 | Remove the up/down "Move up"/"Move down" arrow buttons from the home To Do list rows (`TodoSection`) — the drag handle is enough. **A11y caveat:** those arrows are the *keyboard-accessible* reorder path (37-A added them because drag-to-reorder is pointer-only and a drag-only control trips the jsx-a11y gate — cf. CHANGE-15 / 19-F3). Removing them makes reorder mouse/touch-only; either accept that tradeoff explicitly or add a keyboard alternative (e.g. focus-row + Alt+↑/↓, or keep the arrows visible only on keyboard focus). Frontend-only; decide the a11y call when picked up. | Open | 37-A |

Open: CHANGE-29 (remove To Do up/down reorder arrows — a11y call needed).
