...
| 19-I1 | **Lazy-load + CLS.** `React.lazy` Tiptap + dynamic-import transcribe SDK; reserved-dimension fallbacks; lazy-chunk error boundary + RUM event | Done | 26-A |
...
> **Phase complete (2026-06-18).** All slices are **Done** except **19-C (Rejected)** — pure low-value/high-friction type-safety future-proofing on an already-clean codebase; fixes no known bug, so deliberately not picked up — and **19-K (Closed)** — graduated to [Phase 20](phase-20.md), which shipped. **19-I1 shipped** (PR #300, deploy #602): `React.lazy` editor + dynamic-import transcribe SDK both split out of the entry chunk, reserved-height CLS fallback, lazy-chunk error boundary → `lazyChunkError` RUM event, and the chunk-reload guard now clears after a stability delay (closes the 26-B reload-loop caveat). 19-A/19-B/19-D/19-E/19-F1/19-F2/19-F3/19-G/19-I2/19-I3/19-J all shipped (19-E completed 2026-06-14: #3 PR #285, #1/#2 PR #288; 19-G PR #286). (19-K, the TanStack Query server-state migration, has **graduated to its own [Phase 20](phase-20.md)** — it reverses an Accepted ADR and is 7 slices, too big to sit here.) None blocks the others except as noted (`19-C`→`19-B`, `19-H`→`19-A`, **`19-I1`→`26-A`** — dynamic imports need the zero-downtime frontend deploy first, else lazy chunks 404 mid-session; `19-I2`/`19-I3` carry no such dependency). **19-J, 19-I2, 19-I3 are done (PR #223 / #224 / #221); 19-I1 waits on 26-A.** Value tiers below: **high** = real correctness/UX/security; **medium** = perf/maintainability; **low** = consistency/future-proofing. Because the headline rules are already clean, most slices are medium/low — do not treat the long list as a backlog of bugs.
...
**Status:** Done (PR #300, deploy #602, 2026-06-18). New `LazyNoteEditor` wraps `React.lazy(() => import('./NoteEditor'))` in `Suspense` (reserved-height fallback) + an `ErrorBoundary` (extended with `fallback`/`onError`) that degrades a failed chunk load to a recoverable error and fires `recordRumEvent('lazyChunkError', …)`. The transcribe SDK is `import()`-ed at recording-start in `useTranscription`. `clearChunkReloadFlagAfterStable()` clears the chunk-reload guard after a stability delay instead of synchronously at boot — closing the documented reload-loop window now that a `React.lazy` chunk exists. Build proof: entry chunk carries zero `@aws-sdk`/`smithy` tokens; SDK in a 161 kB lazy chunk, editor in a 532 kB lazy chunk. See [docs/learnings/phase-19i1-lazy-load.md](../learnings/phase-19i1-lazy-load.md).

**(Original plan, for reference.)** **Blocked on 26-A** (zero-downtime frontend deploy): dynamic imports over today's `aws s3 sync … --delete` (`deploy.yml:200`/`:403`) would 404 lazy chunks for a session holding the old `index.html` → mid-session blank pane. Land 26-A first or together.
