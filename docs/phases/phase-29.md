...
| 29-B | **Discoverability affordance.** An always-visible hint below Quick notes plus a `/ai …` row in the `ShortcutsPanel`, so the feature is findable. Pure UX layered on 29-A. (Editor line-highlighting deferred — text affordances were sufficient.) | Done (PR #299, deploy 0d1fe1a) | 29-A |
