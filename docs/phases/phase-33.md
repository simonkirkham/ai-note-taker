...
| 33-B2 | **Re-analysis on the diarized transcript.** Extract the analysis flow (`AnalyseNote`) into a callable service that runs without HTTP scope; the completion Lambda invokes it after appending `TranscriptionDiarized` so summary/tags/actions are gap-filled on the cleaner transcript. | Done (#336, deploy #637) | 33-B1 |
...
- **The analyse intent rides the job name** (`-a`/`-n`, from the auto-analyse toggle at record time). The completion Lambda re-analyses **only** when the flag is set — so an **auto-analyse-OFF** recording is diarized but **never** analysed (it would otherwise get unwanted analysis the user opted out of).
- **The owner's display name** is stamped into `EventMetadata.UserName` on the HTTP create and folded into `NoteDetail.OwnerName`, so the headless re-analysis (no `ICurrentUser`) still attributes action items by name. Old notes (pre-33-B2) have no owner name → the re-analysis prompt sees an empty name (degraded attribution only; unavoidable without a backfill).
- **Manual Analyse during an in-flight job** (auto-analyse ON, user clicks the button anyway) runs locally *and* the job later re-analyses on the diarized transcript → two analyses, **latest (diarized) wins**; summary/instruction-responses are latest-wins and tags/actions dedupe, so it converges. Accepted as a rare edge (the toggle being ON means the user normally lets it auto-run).
