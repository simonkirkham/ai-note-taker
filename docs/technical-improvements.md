...
| TI-73 | **Committing while another session is also committing fails your commit on someone else's tests — the pre-commit gate has no idea anything else is running** | 🔲 **Open** — raised 2026-08-10. Detail in [TI-73](#ti-73-the-pre-commit-gate-is-unbounded-across-sessions) below. |
...
**Note for whoever takes it:** this lands in `.githooks/pre-commit`, the same file as [TI-64] — sequence with that slice rather than racing it. Interim mitigation already in use: agents poll `pgrep -fc 'vitest run'` before committing and treat a timeout in an untouched file as contention, not a defect.
