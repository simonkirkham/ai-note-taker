...
| TI-82 | **Every commit that touches the frontend costs 5-16 minutes of waiting, and the wait buys nothing CI does not already buy** | 🔲 **Open** — raised 2026-08-11 (51-C). `.githooks/pre-commit` runs `npm run test` — the **whole** vitest suite, 101 files / 1042 tests — whenever anything under `web/` is staged. Measured on one slice, one day: **991s, 586s, 424s, 386s, 322s** across five gated commits, ~45 min of pure waiting, and the next pipeline step depends on the commit landing so the wait is serialised, not background. The 3x spread between 322s and 991s is contention from parallel sessions, so it gets worse exactly when the box is busy. **The duplication is the point:** `pr.yml`'s `frontend` job runs the same suite on push, alone on a clean runner, in ~2m19s — so the local run is slower than CI's *and* redundant with it. **Fix:** run only affected specs in the hook (`vitest related --run` on the staged files, or `--changed`), keep the full suite as the CI gate. Keep lint and both typechecks in the hook — they are seconds, and lint is the one gate `tsc`/vitest cannot substitute for. **Do not** simply drop the hook test: it catches the cross-file breakage a changed-files run misses; the point is to pay for that once, in CI, not on every commit. Same productivity shape as [TI-64]. | — |
