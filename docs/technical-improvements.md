...
| --- | --- | --- | --- |
| 1 | `gh workflow list \| grep -i main-checked` | **nothing prints.** A file that does not parse is simply *absent* from the list — absence, not an error message, is the whole failure mode. This is the positive control: the one of the three that can express a no about registration | immediately on merge |
