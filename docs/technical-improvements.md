...
| TI-84 | **A momentary GitHub outage now paints a red X on a `main` commit that did nothing wrong — the exact false-red-X symptom this whole line of work exists to remove** | ✅ **Done** — raised 2026-08-12 (Hawk, PR #471 / [TI-80], should-fix), fixed 2026-08-12 (PR [#474](https://github.com/simonkirkham/ai-note-taker/pull/474)). `scripts/lint-workflows.sh` fetched actionlint with a single `curl` attempt and no `--retry`, so one transient 503 exited 1 — and since [TI-80] that lands on **`main` commits**, where a red X is unattributable and is exactly the signal [TI-69] taught everyone to stop reading. The fetch now retries, with the checksum untouched — and the retry is **bounded**, so a rate-limited runner still fails as a clean, attributable red inside the job's 5-minute budget instead of being killed as an unexplained job timeout. **The `actions/cache` half of the prescription was measured and rejected.** Detail and the four-step proof in [TI-84](#ti-84-what-the-retry-covers-and-why-the-cache-was-rejected) below. |
...
## TI-84. What the retry covers, and why the cache was rejected

**Four things the row's prescription did not say, all measured.**

1. **`--retry 3 --retry-connrefused` alone leaves the most likely blip uncovered.** `--retry` covers only curl's *transient* set — timeouts and HTTP 408/429/5xx. A reset connection, a TLS handshake failure and a DNS miss are all outside it. Against a server resetting the first two of three connections: without `--retry-all-errors`, `curl: (52) Empty reply from server`, exit 52 after **one** attempt; with it, two 52s and then exit 0. It is safe here **only** because the checksum decides whether the bytes are trusted. Its one cost: an absent release (a mistyped `VERSION`) fails after 4 quick attempts instead of 1. Probed for, not assumed — it landed in curl 7.71 (2020) and an older curl rejects the whole command line rather than the single flag.
2. **`--max-time` is applied PER ATTEMPT under `--retry`, not across the set.** Measured: a server stalling the first two of three requests under `--max-time 3 --retry 2 --retry-delay 1` returned exit 0 after **8s**. Keeping `--max-time 120` would therefore have made the worst case 4 × 120s = 8 minutes, **past the job's `timeout-minutes: 5`** — the failure would have stopped being a clean red and become a job timeout. Hence 45s per attempt.
3. **The retry needed a CEILING, and the first version shipped without one — `--retry-delay` does not cap the wait.** curl sleeps for the **larger** of `--retry-delay` and the server's `Retry-After` header, so this PR's original `4 × 45 + 3 × 2 = 186s` was not a bound at all; nothing enforced it and the server chooses. That is a primary path, not an exotic one: GitHub rate-limits release-asset downloads by IP and Actions runners share IP pools, so `429` + `Retry-After` is what a busy runner actually meets. Raised by Hawk on PR #474 and **re-measured independently before and after** (curl 8.5.0, local server, the exact shipped flags):

   | Server response | Claimed by the comment | Measured, before | Measured, after `--retry-max-time 120` |
   | --- | --- | --- | --- |
   | 2 × `429`, `Retry-After: 20` | ~4s | **40.0s**, exit 0 | 40.1s, exit 0 — a wait that fits the budget is still served |
   | 3 × `429`, `Retry-After: 120` | ~6s | **360.3s**, exit 0 | **120.1s**, exit 22 — fails as a clean red |
   | 2 × `429`, no `Retry-After` | ~4s | 4.0s, exit 0 | *(control — proves the header, not the harness, causes the wait)* |

   At 360s GitHub kills the job with "has exceeded the maximum execution time", an annotation naming **neither actionlint nor the download**. So on that path the retry made the red X *slower to appear and harder to attribute* — a fresh instance of the exact defect this row closes. `--retry-max-time 120` fixes it, though **not by the rule it is usually described by**. It is not "no retry starts once 120s have elapsed" — under that rule a `429` at t=0 with `Retry-After: 310` would still be retried and curl would sleep all 310s, past the job kill. curl actually requires that **elapsed plus the sleep it is about to take** fits inside the window, and refuses a `Retry-After` that does not fit **outright** rather than shortening it: measured, `Retry-After: 310` → **one request, 0.011s, exit 22**. The boundary is inclusive — `Retry-After: 120` exactly is still retried once. That is what makes the ceiling real: the last attempt can only *begin* inside the window, and is then bounded by `--max-time 45` — **worst case 120 + 45, measured at 164.1s** (exit 28, retry beginning at 119s against a stalling server), inside `timeout-minutes: 5` with ~2 min of headroom. That is the number now in the script's comment, and it is enforced by a flag rather than asserted by arithmetic. **The generalisable bit:** a timeout you compute from your own flags is a hypothesis; the remote end gets a vote, and only a flag that *refuses* can make it a bound.
4. **The `actions/cache` half of the prescription was rejected, on measurement.** The **whole script** — cold download, checksum, extract, and actionlint over every workflow — is **2.09s** locally and ~1.5s on the runner, while an `actions/cache` restore is a service round-trip plus a tar extract, routinely 1–3s on its own; it is at best break-even. It also reaches the cache service over the network, so it substitutes a network dependency rather than removing one. And it adds a **hard** failure with no self-heal: a corrupt restored `.tools/actionlint-$VERSION` hits `tampered`, which exits 1 by design and deliberately does not re-download, so a transient becomes a red X that stays red until a human busts the key — a fresh instance of the defect this row closes. On the key itself: `VERSION` lives inside `scripts/lint-workflows.sh`, so a literal key duplicates it and can drift (benignly — the cache path is version-stamped, so a stale restore leaves the file absent and it re-downloads), and `hashFiles('scripts/lint-workflows.sh')` avoids the duplication. The key is solvable; the cost/benefit is what fails.

**Proved, not asserted** — four steps, each against a **real** `curl`, with a local server 503-ing the first two requests and a `PATH` shim rewriting only the release URL:

| # | Step | Observed |
| --- | --- | --- |
| 1 | **Red** — before the change | `workflows` job **failure**, run [31622370870](https://github.com/simonkirkham/ai-note-taker/actions/runs/31622370870): one 503, one attempt, `workflow lint FAILED — download failed`, `exit code 1` |
| 2 | **Green** — the same case after | Both jobs **success**, run [31622479628](https://github.com/simonkirkham/ai-note-taker/actions/runs/31622479628): two 503s two seconds apart, third attempt served, lint ran. The retry has been watched retrying |
| 3 | **Injected defect** — `"${retry_flags[@]}"` removed | Flaky case flips back to exit 1; healthy case unmoved at exit 0 and **one** request. Both halves checked, then reverted |
| 4 | **Integrity still fires** | Wrong pinned tarball sha → `ABORTED — checksum mismatch`; wrong pinned binary sha → `ABORTED — cached ... does not match`; and a **successful** retry onto 200 KB of `/dev/urandom` → `ABORTED — checksum mismatch`. Retries cannot launder bad bytes |

**Not done, deliberately:** a permanent hermetic self-test of the retry (the [TI-77] `scripts/test-merge-gate.sh` precedent) would stop someone deleting the flags later. Making it hermetic means stubbing the pinned checksums as well as the download — a larger surface than the one line it guards, plus a step on every push to `main`. File it as its own row if wanted.
