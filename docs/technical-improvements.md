...
**Two clocks, because the obvious one is the one that lied.** On #762 `updated_at` froze 59 seconds *before* the last job finished — it fails **early**, in the direction that admits a discount sooner than the truth warrants. Keying staleness solely on it would have inherited the defect being fixed, so the newest job completion must be old as well; that comes from a different part of the API and was correct on #762.
...
**CI cannot prove this by a green PR** — `pr.yml` paths-ignores `scripts/**`. Evidence: **47 stub-driven cases** in `scripts/test-merge-gate.sh` (25 of them TI-81), wired into `docs-check.yml` by [TI-77] and confirmed *running* in CI by name; the live readings above; and **eight injected defects**, each flipping only the cases that claim it while the targeted orphan case stays green.
