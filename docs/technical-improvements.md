...
**Update 2026-07-01 — recurred on deploy #686 (Phase 46-A), both cold attempts, with an empty cards list.** Attempt 1 failed `FilterBackNavigationJourney.Filter_OpenNote_Back_RestoresFilter`, attempt 2 failed `NoteDeleteJourney.Deleted_note_is_not_in_list_after_navigating_back_manually` — **different journeys, identical symptom**: `cards(0)=[]` (the *entire* list empty, not just the new card missing) with 5×`200 /notes/cards`. Passed clean on attempt 3. The empty-list shape reinforces the workspace-context-on-reload hypothesis (a cold projector would lag one card, not blank the whole list) — the reload landed on `/w/__default__/notes/{id}` yet the list came back empty. 46-A is frontend markdown-rendering only and touches none of the cards path, confirming this is env/gate, not a regression. Two consecutive cold-start reruns failing (vs the historical ~1/10) suggests the E2E projector was scaled-to-zero this session; TI-52 (projector keep-warm) would remove the cold window that makes this reproduce.
