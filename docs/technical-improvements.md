...
**Evidence for the round:** all seven loops verbatim, green before and red after, under gawk 5.2.1 **and** a mawk 1.3.4 shim (`ubuntu-latest` resolves `awk` to mawk); the seven new fixtures seen **red against the pre-fix checker** and green after, with the 20 pre-existing cases unmoved in both directions; and a positive control on the default `git ls-files` path — a `ps aux` loop appended to a tracked script named the file and line, and the revert returned `OK`.
