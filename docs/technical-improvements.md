...
| TI-83 | **A bug can be left half-closed — filed away as fixed while still listed as open — and the commit that does it runs no check at all, because a commit touching only the archive file matches neither of the two filters deciding whether the check runs** | ✅ **Done** — raised 2026-08-11 (Hawk, PR #471 / [TI-80], classified should-fix), fixed 2026-08-13 (PR [#477](https://github.com/simonkirkham/ai-note-taker/pull/477)). **Latent, not observed:** no archive-only commit has ever landed, so this never bit — it is a hole, demonstrated on purpose (`proof/ti83-paths`, `1cf9468d`), not a miss. `scripts/check-doc-ids.sh` reads `docs/phases/phase-bugs-archive.md` for two of its checks (duplicate `## BUG-N` entries; a bug living in *both* the live doc and the archive) and that file was in **neither** of `docs-check.yml`'s two `paths:` lists. **Fixed by subtraction, not a checker:** the `paths:` list is deleted from the **push** trigger, so both jobs always run on a push to `main`; `pull_request` keeps its list and gains the archive path by hand. Detail in [TI-83](#ti-83-why-subtraction-beat-keeping-two-lists-in-step) below. |
...
**What it would cost:** a bug filed away as fixed while its row still sits in the open list — half-closed, and nothing says so. `scripts/check-doc-ids.sh` uses `docs/phases/phase-bugs-archive.md` for two of its four checks (duplicate `## BUG-N` entries; a bug present in both the live doc and the archive), and the file was listed in neither `paths:` filter. So a commit touching **only** the archive matches neither trigger and runs no check at all — and archive-only is precisely the shape of a half-done close, where the row was never deleted from `phase-bugs.md`. Archiving is a Scribe step that commits straight to `main`, the one route with no other guard.

**Latent, not observed — the distinction matters for anyone sizing a future `paths:` decision against this.** It has not happened. Measured over the full history of `origin/main`: **11** commits have ever touched `phase-bugs-archive.md`, and **all 11** also touched `phase-bugs.md`, which was already in the `pull_request` list — so every one of them matched. Separately, the `push:` trigger did not exist at all until `14c6c034` (2026-08-12, [TI-80]), which is *after* the most recent of those commits (`bb4c5611`, 2026-08-11). Since the trigger landed there have been zero archive commits, so the gap has never been exercised in either direction.

**Negative control — the gap demonstrated on purpose.** On `proof/ti83-paths`, commit `1cf9468d` changed only `phase-bugs-archive.md` and added a duplicate `## BUG-1` heading (the file then carries the heading at both line 11 and line 415 — exactly what `check-doc-ids.sh` exists to catch). It produced **no run at all**: that branch's only run is `31623178252`, on its parent `8589d591`. That parent is not evidence for this change — it still carried the full push `paths:` list and fired by matching `.github/workflows/**`. **The branch has since been deleted** (CLAUDE.md step 13), so `1cf9468d` no longer resolves; the durable, still-checkable form of this evidence is the Actions run list for `proof/ti83-paths` — one run, on the parent, none on the archive-only commit.

**A second, real instance on `main` — the filter demonstrably suppresses the check on live traffic.** Today (2026-08-13), re-measured at 09:35 BST immediately before merge: **14** commits landed on `main` and only **12** produced a `Repo Checks` push run. Counted by diffing `git log origin/main --since=2026-08-13T00:00:00Z --until=2026-08-14T00:00:00Z --pretty='%h %s'` against `gh run list --workflow docs-check.yml --event push --branch main --limit 40 --json headSha,conclusion`. (An earlier draft read 10 and 8; `main` moved four commits during review — the two absentees are unchanged.) `fb286458` touched one file, `tests/EventStore.Integration/EventStore.Integration.csproj`, matched nothing in the push list, and produced a `Deploy` run but **no** `Repo Checks` run. It carried no defect, so nothing was missed — but it proves the suppression is real rather than theoretical. The other absentee, `4727672f`, produced no runs at all and is [TI-90], not this.

**Why the list went rather than a checker keeping the copies in step.** The list never scoped what got *checked* — `actionlint` lints every workflow in the tree and `check-doc-ids.sh` greps every tracking doc, whatever a commit touched. It only decided whether the check *ran*. That makes it pure drift surface with no upside, and Actions has no YAML anchors, so keeping it meant typing the same list twice and hoping. Deleting it closes every future instance of the drift, not just the one found. `pull_request` keeps its list — there the filter earns its keep, and drift on that side is loud: a check visibly stops appearing on PRs.

**The limit, stated so nobody reads this as more than it is.** Dropping `paths:` makes the workflow run whenever a push **event** arrives. It does nothing when no push event arrives. A commit on `main` that triggers no check-suites at all is a separate defect — squash-merge `4727672f` produced **zero** runs of any workflow, despite touching three paths that were in the old push list, while the control `fb286458` (also a squash merge, same committer) produced its `Deploy` run normally. That is tracked as [TI-90], owned elsewhere; TI-83 does not fix it, and after this change `4727672f` would still produce nothing.

**Deploy-time cost, and the bill.** Deploy wall clock is **unchanged** — independently verified: these two jobs do not gate `deploy.yml` and run in parallel with it, so no merge waits any longer. Per-**job** duration, re-measured over the last twelve `Repo Checks` push runs on `main` (2026-08-13): `doc-ids` **19–23s**, `workflows` **5–10s**, the two in parallel. (An earlier draft quoted "23–28s each" — that was the *run* wall clock, not each job.) **The recurring bill is £0.** Actions bills per job rounded up to the minute, so two jobs would be ~2 minutes if billed at all — but `gh repo view --json visibility` returns **PUBLIC**, and standard-runner minutes are free on public repos. Volume added is small regardless: of today's 14 commits on `main`, exactly **one** (`fb286458`) fell entirely outside the old list, so the fix turns 12 runs into 13.

**The one real cost this added, and why it is already closed.** Every run of the `workflows` job re-downloads actionlint, and `scripts/lint-workflows.sh` used to fetch it with no `--retry` and no cache on `.tools/` — so one transient network blip exits 1 and paints a red X on a `main` commit that did nothing wrong. More runs on `main` means more exposure to that, which would have been this change making a red-X problem worse while fixing a different one. **[TI-84] fixes exactly it, and landed first:** PR [#474](https://github.com/simonkirkham/ai-note-taker/pull/474) merged 2026-08-13 08:12 UTC as `608c882e`, and this branch is rebased onto it. Verified on `origin/main` after the rebase: `lint-workflows.sh` now builds a `retry_flags` array and probes `curl --help all` before adding `--retry-all-errors` / `--retry-connrefused`. So the interaction is **resolved, not accepted**.

**Post-merge confirmation — a Scribe step, because the core mechanism has never been watched work.** A push to `main` with **no** `paths:` firing this workflow has not been observed. Neither existing run proves it: `31623178252` ran against a file that still had the full push list, and this PR's own merge commit touches `docs-check.yml` and `docs/technical-improvements.md`, both in the *old* list, so its push run proves nothing new either. So after merge, wait for the next push to `main` whose files **all** fall outside the old list — a Scribe learnings doc, `CLAUDE.md`, `docs/token-log.md`, `docs/human-input-log.md`, a `.csproj`, or an archive-only commit — and confirm a push run exists for that exact sha:

```bash
gh run list --workflow docs-check.yml --event push --branch main --limit 5 \
  --json headSha,createdAt,conclusion
```

**What failure looks like:** if that sha is absent from the output, the trigger did not fire and the change did not take effect — the run list is the only place this shows, because a workflow that never runs prints nothing anywhere else. Widen `--limit` before concluding it is absent; do not read a short window as a negative.

Expect this to resolve within hours, not days: measured today (2026-08-13), 14 commits landed on `main` and 1 qualified, at a cadence of ~14 commits/day.

**Found while rebasing this branch: finished work can come back from the dead and read as outstanding again.** Three times during this PR's rebases, the `merge=union` driver on `docs/technical-improvements.md` silently restored a `## TI-` section that had just been archived — `## TI-84.` and `## TI-73.` both reappeared with no conflict and no marker, so nothing on screen said anything had happened. `scripts/check-doc-ids.sh` structurally cannot see it: its live-vs-archive check compares **BUG** ids only, which is [TI-92](#ti-92-half-archived-ti-items-are-invisible-because-the-live-vs-archive-check-covers-bugs-only). The resurrection itself — union restoring content one branch deleted, against another branch's surrounding context — is [TI-94](#ti-94-a-merge-silently-restores-a-deleted-section-and-nothing-detects-it), which records this branch's `## TI-84.` case as its instance 2 and specs the orphan-section check that would catch it. Until that lands the defence is manual, and it belongs in every merge touching this file:

```bash
git diff origin/main -- docs/technical-improvements.md | grep '^-' | grep -v '^---'
```

Zero deletions other than the ones your own change makes. That one reading catches a resurrection and a clobber of someone else's content at the same time, and unlike a heading-set comparison it needs no id list and no regex to be right. It is the gate that caught this branch immediately before merge; keep the heading-set diff alongside it for diagnosing *what* moved.

**Verified, not asserted.** `scripts/lint-workflows.sh` exit 0 with no findings; `scripts/check-doc-ids.sh` printed `doc ids OK`; `actionlint` on the changed file alone exit 0; the YAML read back shows `push` with no `paths` key, `branches: [ main ]` on both triggers, the archive path present in `pull_request.paths`, and both jobs still declared. **Positive control:** TI-69's exact defect injected into a copy drove actionlint red (exit 1, `parser did not reach end of input after parsing the expression. 2 remaining token(s) in the input: "*", "INTEGER"`) — the instrument reports the condition it is trusted to exclude. That control is the point here: a workflow that fails to parse produces no run at all and is indistinguishable from one that was filtered out. **Negative control:** `1cf9468d` on `proof/ti83-paths`, above. **Still unobserved, and honestly so:** a push to `main` firing this workflow with no `paths:` — see the post-merge Scribe step above.

**Depends on:** [TI-84] (PR #474, merged `608c882e` 2026-08-13) — landed first, and this branch is rebased onto it. See the cost note above.
