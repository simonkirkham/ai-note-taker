...
**CI cannot prove this by a green PR** — `pr.yml` paths-ignores `scripts/**`. Evidence is `scripts/test-merge-gate.sh`, **50 cases** (28 of them added here), which `docs-check.yml` runs by name since [TI-77] — observed green in run [31539933340](https://github.com/simonkirkham/ai-note-taker/actions/runs/31539933340), with the two newest cases named in its log rather than inferred from a tick; the live readings above; and **ten injected defects**, each flipping only the cases that claim it while the targeted orphan case stays green. Two of those injections exist because review found guards that were shipping untested — the newest-run sort, whose failure mode was a `GREEN` on a window whose newest run had **failed**, and a `JOBFAIL` message still accusing main for a run that was not main's latest.
