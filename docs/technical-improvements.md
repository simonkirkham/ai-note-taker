...
| TI-69 | **Every push now leaves a red X on the commit — about 20 so far — and none of them means anything is broken** | 🔲 **Open** — raised 2026-08-10. Since ~14:34Z on 2026-08-10, EVERY push (main and branches alike) creates a failing `E2E (on demand)` run. Runs #142-#161 and counting, all `event=push`, all `conclusion=failure`, **all with zero jobs** — so nothing ran and no test failed. Zero jobs plus GitHub displaying the run as the file path `.github/workflows/e2e.yml` rather than its declared `name: E2E (on demand)` is the signature of a workflow GitHub could not load or validate on that commit, not of a test failure. The file on `main` declares `on: workflow_dispatch` only and has not changed since `01833e35`, so the trigger is coming from somewhere else — most likely a second workflow file or a repo/org-level setting added around 14:30Z; that is the first thing to check. **Why it matters:** the deploy gate stays green, so nothing is blocked — but every commit now carries a red mark, which is exactly the signal that teaches people to stop reading CI. It also burns a runner per push and, because `e2e.yml` shares the **`deploy` concurrency group**, a run that ever did start could cancel a pending deploy (the 34-B route-contract hazard that workflow's own comment documents at length). **Fix direction:** identify what added the push trigger, remove it (the workflow is deliberately on-demand: it exists so flake counts do not need no-op commits), and confirm pushes stop producing runs. **Evidence:** `gh run list --workflow e2e.yml --limit 20 --json number,event,conclusion` — every row `push`/`failure`; `gh api repos/simonkirkham/ai-note-taker/actions/runs/31404730676/jobs` returns `total_count: 0`. |
