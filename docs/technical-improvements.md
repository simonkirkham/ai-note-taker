...
|------|---------|--------|------------|
| TI-106 | **A change can look saved and reviewed when nothing was actually sent**, so the version that ships is an older one. In a worktree left on a detached HEAD, `git push origin <branch>` pushes the *branch ref* rather than the commits in hand: it exits 0, prints nothing, and three rounds of review fixes stayed local while the merge gate reported green on checks from a two-rounds-old commit. Caught only by disbelieving a green that arrived 30s after a push. Fix: have the push helpers (or a `scripts/` wrapper) compare the remote ref against `HEAD` afterwards and fail loudly when they differ. Found 2026-09-21 during [BUG-88]; pairs with [TI-105], which is the same blindness one layer up. | Open | — |
...
| TI-105 | **The merge-safety check can say a change is safe to merge while it is reporting on an older version of that change** — it reads the pull request's check results without asking which commit they ran against, so a branch whose newest commits never reached the server (a detached checkout pushes a stale ref and still exits 0) passes a gate that examined code from three rounds earlier. Compare the checks' commit against the branch head in `scripts/merge-gate.sh` and refuse the mismatch. Found 2026-09-21 by a parallel session during [BUG-88]; same class as the filter fix in `flake-watch.sh` — a check that answers confidently about the wrong thing. | Open | — |
...
**Severity:** Medium. Losing the transcript is the bug; this item makes the next occurrence diagnosable in minutes, from the server alone.
