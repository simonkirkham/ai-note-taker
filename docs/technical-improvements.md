...
| 2 | Stalled record | run not `completed`, **and** every job `completed`+`success`, **and** `pending_deployments` empty, **and** `updated_at` older than 10 minutes |

Everything else blocks and prints the raw values it saw. A terminal-but-**failed** job is reported `NOT SAFE`, naming the job — never discounted.

**The form is the point, and it is what review changed.** Three separate clauses of this condition moved rather than closed in one afternoon: a job count, then pending environment approvals, then job conclusions — where "terminal" silently admits `failure`, which would have reported **GREEN on a run whose `deploy-production` failed**. Three positions, one shape: a value that is absent, unexamined, or admits more than its name suggests. Excluding the unsafe states means enumerating everything that can go wrong, and that enumeration had already failed three times in an afternoon; an allow-list fails **closed** on anything unanticipated, which is the property a merge gate wants.

**Clause 2 of state 2 is latent, not live.** No protection rules exist on `deploy-test` or `deploy-production` today, so `pending_deployments` returns `[]` on every run — checked on #762 and #763. It is required anyway: `deploy.yml` declares `environment:` on both, so the day someone adds a required reviewer or a wait timer, a run can sit awaiting approval with **no job record for the gated job at all** — every created job finished, nothing transitioning, `updated_at` going stale. The other clauses would both hold while the deploy is genuinely pending. Same absent-value shape as the rest: *"every job is finished" reads identically whether nothing is left or the next thing has not been allowed to start.*

**Cost:** two extra `gh api` calls (`/jobs`, `/pending_deployments`) **per not-finished run only**, short-circuiting so a run with a job still executing pays one. A quiescent window — the normal case — pays nothing.

**The discount is printed on the verdict line, never silent.** A gate that quietly ignores runs is worse than one that blocks, because nobody can tell it is doing it.

**CI cannot prove this by a green PR** — `pr.yml` paths-ignores `scripts/**`. The evidence is the 20 stub-driven cases added to `scripts/test-merge-gate.sh` (wired into `docs-check.yml` by [TI-77]), the live reading above, and an injected-defect run showing the guard cases going red while the targeted case stays green.
