...
**CI cannot prove this by a green PR** — `pr.yml` paths-ignores `scripts/**`. Evidence is `scripts/test-merge-gate.sh`, **52 cases** (29 added here; `main` has 23, 0 removed), which `docs-check.yml` runs by name since [TI-77] — observed green in run [31539933340](https://github.com/simonkirkham/ai-note-taker/actions/runs/31539933340), with the newest cases named in its log rather than inferred from a tick; the live readings above; and **eleven injected defects**, each flipping only the cases that claim it while the targeted orphan case stays green.
