...
**What it costs:** the gate says `BLOCKED — rebase/resolve conflicts` on a branch that is perfectly clean, so the next move is a pointless rebase — or worse, blaming whoever wrote the branch for a conflict that never existed. Hit on PR #460 on 2026-08-11, moments after two merges landed on main.

**Why:** `scripts/merge-gate.sh:21-26` reads `mergeable`/`mergeStateStatus` once and treats *every* value other than `MERGEABLE`/`CLEAN` as a conflict, printing a fixed remedy. GitHub returns `UNKNOWN`/`UNKNOWN` while it is still computing mergeability — routine for a minute or so after main moves — which is not a conflict and not an error. Re-polling #460 three times returned `MERGEABLE`/`CLEAN` every time.

**Fix:** treat `UNKNOWN` as *not yet known*: re-poll a few times with a short sleep before reporting anything, and if it is still `UNKNOWN`, say so in those words rather than naming a cause. Reserve the rebase advice for `CONFLICTING`.

**Class:** a check whose failure message asserts a cause it has not established. Same shape as gating on `reviewDecision` (empty by design here, because Hawk posts verdicts as comments) — the answer is plausible, actionable and wrong, and it sends the investigation to the accused instead of the instrument.
