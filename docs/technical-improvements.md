...
**Outstanding (5 Open + 3 Partly):** Auto-backfill projection on deploy; `NoteSearchView` tombstones (verify/close); `WorkspaceList` GSI; Generalise append-retry; `NoteEditor` ordering test; *(partly)* ESLint import-resolver + typed-lint (jsx-a11y done via 19-F3); state-mgmt colocation; deploy-credentials root cause.
