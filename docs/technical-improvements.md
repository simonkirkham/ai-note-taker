...
| TI-41 | Fold the `GetActions` cross-stream re-poll into `ConsistencyGate` (existence wait)            | 🟡 **In PR, pending deploy** — raised 2026-06-14 (`Task.Delay` audit), actioned 2026-06-15 via **Option B**. `GetActions` had a hand-rolled `Task.Delay(100)×10` loop *beside* the gate to ride out note-vs-action cross-stream projector lag. Added `IConsistencyGate.WaitForPresenceAsync<T>` (bounded presence-poll, shares the version wait's interval/cap/delay/logging) and replaced the loop with it. Scribe → ✅ on deploy |

**Outstanding (7 Open + 4 Partly):** TI-17 Auto-backfill projection on deploy; TI-20 `WorkspaceList` GSI; TI-23 Generalise append-retry; TI-25 `NoteEditor` ordering test; **TI-33 `NoteCardList` Scan→GSI**; **TI-34 Lambda naming audit**; **TI-40 read-only creds for automated cloud observability-review**; _(partly)_ TI-7 ESLint import-resolver + typed-lint (jsx-a11y done via 19-F3); TI-3 state-mgmt colocation; TI-24 deploy-credentials root cause; **TI-41 `GetActions` presence wait (in PR, pending deploy)**. **TI-39 (chronic E2E deploy-gate flakiness) is done — 2026-06-13, four stacked causes incl. [BUG-27]/[BUG-29]; residual [BUG-28] carved out. The 2026-06 cold-start trio (TI-32 priming + TI-35 ReadyToRun + TI-36 512 MB) is done — #260/#270, deploys #552/#562 — cold p50 7.92→2.24 s (−72%). The 2026-06 observability triad (TI-37 RUM resource-error capture, TI-38 error-log-level, BUG-23 rebuild-timeout 503) is done — #267/#268/#269, deploys #556/#557/#558; the 2026-06 dependency upgrade audit (T1/T7/T2/T3/T4 = TI-27/28/29/30/31) is fully cleared.**
...
🟡 **In PR, pending deploy** (actioned 2026-06-15 via **Option B**; Scribe flips to ✅ on the deploy that ships it).
