...
| TI-53 | `desktop/tests/*.spec.ts` never run in CI — the desktop suites are hand-run only | 🔲 **Open** — raised 2026-08-06 (Hawk, PR #417/CHANGE-37). No workflow executes the desktop Playwright specs: `publish-desktop.yml` only does `npm ci` + `npm run package`, and `pr.yml`/`deploy.yml` never touch `desktop/`. So `displayMedia`, `spellCheckMenu`, `streamingTranscript`, `whisperParse`, `localEngine`, `models`, `server`, `diarizeMerge`, `packaging` and `publish` specs all pass or fail **invisibly** — a regression in the pure decision logic behind the Electron seams (display-media selection, spell-check menu, the [BUG-53] streaming reducer, whisper output parsing) reaches an installer with nothing red. That layer is deliberately extracted **so that it can** be tested headlessly, so the seam already exists and only the runner is missing. It also raises the cost of any mutation-coverage gap in those suites, since nothing re-checks them. Fix: add a `desktop` job to `pr.yml` running `npm --prefix desktop ci` + `npx playwright test` (no `playwright install` needed — these are pure-function specs, no browser). **Deploy-time: none** — PR CI only, parallel to the existing jobs. Medium urgency — the desktop shell now carries real logic (Phase 31, 48-A/B/C/E, BUG-52, BUG-53) rather than thin wiring |
...
> **The smoke test must resolve the binary through `whisperServerBinPath()` — the same code path production uses — not through a hand-supplied env var.** A test that reaches its dependency by a private route proves only that the dependency works, never that the app reaches it.
