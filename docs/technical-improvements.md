...
| TI-67 | **Every `recordRumEvent` call in the app is inert — RUM custom events are DISABLED on the app monitor** | 🔲 **Open** — raised 2026-08-10 (BUG-60 review). `recordRumEvent` calls `cwr("recordEvent", …)`, which requires custom events to be enabled on the CloudWatch RUM app monitor. Verified in prod: `aws rum get-app-monitor --name notetaker-rum --profile prod --region eu-west-2` returns `"CustomEvents": {"Status": "DISABLED"}`, and `CustomEvents` appears nowhere in `NoteTakerStack.cs` or `tests/Infrastructure.Assertions/` — AWS defaults it to disabled. So **every** custom event ever added is a no-op in production: `deadNoteLink` (21-C), `tabRestoreFailed`, `noteTabOpened` (49-B), `lazyChunkError`, `deletedNoteRescue` ([BUG-59]) and `authStorageBlocked` ([BUG-60]). Each was written, reviewed and shipped as an observability signal, and none of them has ever emitted. **Fix:** add `CustomEvents = new CfnAppMonitor.CustomEventsProperty { Status = "ENABLED" }` to the `CfnAppMonitorProps` in `NoteTakerStack.cs`, plus an `Infrastructure.Assertions` case so it cannot silently regress. **This is an INFRA change**, so the "merged + a green deploy ≠ live in prod" guardrail applies: confirm the shipping deploy had `backend=true`, then re-check `get-app-monitor`. One fix unblocks six signals at once. |
...
> Items carry stable IDs `TI-1`–`TI-65` in claim order (the `ID` column above); each detailed section below repeats its ID. **Never hand-pick the next id — run `scripts/next-doc-id.sh ti`.** Reference an item as `TI-N`. The dep-audit `T#` tags are retained in parentheses for cross-reference with the audit report.
...
| TI-68 | **The pre-commit hook's vitest run is unreliable on WSL — capping worker threads makes it both reliable AND ~35% faster** | 🔲 **Open** — raised 2026-08-10 (51-B). The hook runs the full 1030-test suite on every commit touching `web/`. Unconstrained on WSL it fails non-deterministically with scattered `Test timed out in 5000ms` errors on files that cannot fail for a real reason (`taskListMarkdownRoundTrip.test.ts` is a pure string function with no async at all). **Measured on 51-B, same commit, same tree:** unconstrained → 8 failed / 1214 s, then 33 failed / 835 s; `VITEST_MAX_THREADS=3` → 1030 passed / 154 s, but **3 is marginal** — a later run at 3 still failed (5 failed / 714 s, timeouts in `ContextMemoization`, `ListView`, `useDocumentTitle`, none touched by the change). `VITEST_MAX_THREADS=2` → **1032 passed / 139 s**, and was the only setting that passed first time. Across 51-B the hook blocked 5 commit attempts and cost ~90 min without ever finding a real fault. Capping is *faster*, not a tradeoff — over-subscribed vmThreads workers thrash on the Windows FS (`setup` alone was 9179 s cumulative in the failing run vs 872 s capped). Cost 3 blocked commit attempts, ~50 min, on a change that was green throughout. **Do NOT "fix" this by forcing `CI=true`** — that selects the `forks` pool, which `web/vite.config.ts` documents as segfaulting on WSL; tried during 51-B and it was worse (53 `Failed to start forks worker` errors, 44 of 97 files never ran). **Fix:** set `poolOptions.vmThreads.maxThreads` in `web/vite.config.ts` for the non-CI branch only (CI uses `forks` and is unaffected), so every clone gets the reliable path without needing to know the env var. Pick the cap from core count rather than hard-coding 3. |
| TI-69 | **Every push leaves a red X on the commit, and the workflow behind it has never once run — not in three days, not ever** | 🔲 **Open** — raised 2026-08-10. `E2E (on demand)` (`e2e.yml`, added 2026-08-07 in `d0a92709`/#429) has produced **163 runs, every one triggered by `push`, every one `failure`, none with a single job**. Its first run failed. There has never been a successful run, and there has never been a `workflow_dispatch` run — nobody has invoked it the intended way. `gh run view` says "This run likely failed because of a workflow file issue"; the run and the workflow both display as the file path rather than the declared `name: E2E (on demand)`. The file declares `on: workflow_dispatch` only, parses as valid YAML, and has not changed since 2026-08-08. **Why it went unseen for three days:** it blocks nothing (deploy gate and PR checks are separate workflows and stay green); `scripts/deploy-status.sh` reads only `deploy.yml` and `scripts/merge-gate.sh` only that plus the PR's own checks, so **the project's own CI-health tooling structurally cannot surface this workflow**; and its runs render as a path, so a listing reads as noise. **The sharpest consequence:** CLAUDE.md recommends `gh workflow run e2e.yml -f runs=10` **twice** as the way to prove a flake fix without no-op commits — the documented technique has never worked. [BUG-66] needed that bar on 2026-08-10, recorded the script as "does not apply", and counted PR-CI runs by hand instead: the broken tool was routed around rather than discovered. Nothing verified the workflow ran before the same PR wrote it up as standing guidance. **Fix direction:** find why GitHub rejects the file (its own message points at the workflow file, despite local YAML validity — suspect the `on:` key being parsed as boolean `true`, or an inputs/`fromJSON` construct GitHub validates at load), fix it, then confirm both that a dispatch run succeeds AND that pushes stop producing runs. **Do not close on a green dispatch alone** — this row exists because a workflow was written up as working without one. **Evidence:** `gh run list --workflow e2e.yml --limit 200 --json event,conclusion` → 163 push/failure, nothing else. |
| TI-70 | **Nothing stops a broken workflow file reaching `main`, so the next one also shows up as an unexplained red X on everyone's commits rather than as a failed check** | 🔲 **Open** — raised 2026-08-10, out of [TI-69]. That defect survived 3 days and 162 red marks, and was first written up with a confidently wrong root cause, because an unparseable workflow does not fail like a test: it produces a zero-job run named by its file path, attached to a push the workflow does not even subscribe to, with no annotation on the PR that introduced it. **Cost:** every commit in the repo carries a false red X until someone happens to run the one command that reveals the parse error, and the affected workflow silently does not exist in the meantime. **Fix direction:** run `actionlint` (single Go binary, ~2s, no runtime deps) over `.github/workflows/**` and `.github/actions/**` in `docs-check.yml`, which is the workflow that already exists to gate paths `pr.yml` ignores — note `pr.yml` paths-ignores nothing relevant here, but `docs-check.yml` is the cheaper host. Add the same call to `.githooks/pre-commit` for local feedback, on the same terms as `check-doc-ids.sh` (CI is the real gate; the hook needs per-clone `core.hooksPath`). actionlint catches the exact class — expression syntax, unavailable contexts, unknown keys — plus shellcheck over `run:` blocks, of which this file has many. **Verify with:** re-introducing `${{ fromJSON(inputs.runs) * 4 + 15 }}` must fail the check. |
| TI-71 | **A PR can go red with 26 failed tests that were never run, blocking a merge on a Docker Hub hiccup rather than on anything in the diff** | 🔲 **Open** — raised 2026-08-10 (PR #454). The `eventstore` check failed 26 of 37 with `Docker.DotNet.DockerImageNotFoundException: No such image: amazon/dynamodb-local:1.21.0`, thrown at `DynamoDbFixture.InitializeAsync` container start. **Not a missing tag and not the diff:** `1.21.0` is present on Docker Hub (confirmed via the registry tags API), the same `eventstore` job passed on the three preceding PR runs, and the failing PR changed only `.github/workflows/e2e.yml` and a docs row — no C#, no test file. Testcontainers reports a pull that did not land as an image that does not exist, which is what Docker Hub's unauthenticated rate limit looks like from inside a GitHub runner (shared egress IPs, anonymous pulls). A re-run of the failed job went green with no code change. **Cost:** every occurrence is a false red on someone's PR, a blocked merge gate, and a re-run — and the message actively misleads, since it names an image tag that is fine. **Fix direction, cheapest first:** (1) authenticate the pull — `docker/login-action` with a Docker Hub token raises the limit substantially and is a secret plus three lines in `pr.yml`; (2) or pull from a mirror not subject to that limit (ECR Public hosts a DynamoDB Local image); (3) or wrap container start in a bounded retry so a single transient pull does not fail 26 tests. (1) is preferred — it removes the cause instead of masking it, and the retry in (3) would still burn the job's time on every hit. **Note the shape:** the failure is at *fixture* level, so one pull failure is reported as 26 independent test failures — worth remembering before reading a count like that as a real regression. |
