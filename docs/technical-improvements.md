...
| TI-84 | **A momentary GitHub outage now paints a red X on a `main` commit that did nothing wrong — the exact false-red-X symptom this whole line of work exists to remove** | ✅ **Done** — raised 2026-08-12 (Hawk, PR #471 / [TI-80], should-fix), fixed 2026-08-12 (PR [#474](https://github.com/simonkirkham/ai-note-taker/pull/474)). `scripts/lint-workflows.sh` fetched actionlint with a single `curl` attempt and no `--retry`, so one transient 503 exited 1 — and since [TI-80] that lands on **`main` commits**, where a red X is unattributable and is exactly the signal [TI-69] taught everyone to stop reading. The fetch now retries, with the checksum untouched. **The `actions/cache` half of the prescription was measured and rejected**, and two of the row's numbers were wrong; detail and the four-step proof in [TI-84](#ti-84-what-the-retry-covers-and-why-the-cache-was-rejected) below. |
...
**Three things the row's prescription did not say, all measured.**
