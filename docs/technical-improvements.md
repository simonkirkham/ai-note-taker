...
| TI-42 | Residual cold-start E2E flake — `NoteReadYourWritesJourney` cards list (~1/10)            | 🔲 **Open** — surfaced 2026-06-16. Points at **workspace context on reload** (`GetNoteCards` filters `currentWorkspace.Includes`). **Update 2026-06-17: did NOT recur in 13+ consecutive green runs** (not proven fixed, just not reproduced). A safe **client-side** diagnostic is now committed (PR #292, `AppPage.cardsRequestLog` — records sync request URLs only, never the body) so the next failure surfaces the requested `/w/{wsId}/` prefix in the gh run log. Next: catch a live failure, confirm the workspace mismatch, fix the reload-time workspace context |
...
✅ **Done** — #289, deploy #589 (2026-06-15) via **Option B**. E2E gate (incl. `ActionReadYourWritesJourney`, which exercises this cross-stream wait) passed through to production.

**Problem.** `ActionItemHandlers.GetActions` carried its own raw polling loop — `for (… attempt < 10) { await Task.Delay(100, ct); … }` — *in addition to* the `IConsistencyGate.WaitAsync` call above it. The gate waits on the **action** stream's projected position; the ownership check then reads the **note**'s `NoteDetail` projection, a *different* stream. DynamoDB Streams give no cross-key ordering, so the action can fold before the note projection is built → a spurious 404 even though the gate returned `Fresh`. The hand-rolled loop rode out that lag but duplicated the gate's job with a worse impl: fixed 100 ms, a real `Task.Delay` that *sleeps in unit tests*, no cap result, no observability.

**What the read actually needs.** `GetActions` reads the note projection for **authorization only** — existence + `UserId` match. Ownership is immutable (set at note creation). So it needs *presence* of the note projection, not a specific note-stream version.

**Fix (Option B — existence wait in the gate).**
- Added `IConsistencyGate.WaitForPresenceAsync<T>(Func<CancellationToken, Task<T?>> read, …)`: bounded-polls `read` until non-null (returns it) or the cap elapses (returns null), sharing the version wait's poll interval / cap / injected delay / slow-and-absent logging.
- Replaced the `Task.Delay` loop in `GetActions` with `gate.WaitForPresenceAsync(c => noteDetailStore.GetAsync(noteId, c), ct)`. Null → 404, exactly as before.
- Net: the only flagged `Task.Delay` smell from the audit is gone; the cross-stream wait is now virtual-time-testable and observable, with no client/token/Command-Lambda change.

**Why not the other two options:**
- **Option A — multi-position token (`action@N,note@M`)** — rejected for this read. The Command Lambda validates the note via its *projection*, not the note *event stream*, so it doesn't know `M`; surfacing it adds a write-path event-store read to wait on a version the read doesn't need (presence ⊇ ownership at v1). Right shape only for a future read that needs a second stream's *content*; build it then.
- **Option C — denormalize the owner `UserId` into the action projection** — would remove the second wait entirely (authorize off the already-gated action stream), but costs an `ActionItemAdded`→v2 event version + deserializer narrowing + rebuild/backfill. Deferred; revisit if cross-stream auth recurs in more read paths.

**Files:** `src/Api/Consistency/IConsistencyGate.cs`, `src/Api/Consistency/ConsistencyGate.cs`, `src/Api/Handlers/ActionItemHandlers.cs`, `tests/Api.Integration/ConsistencyGateTests.cs` (3 presence tests: present-immediately, appears-after-N-polls, never-appears→null-at-cap).

**Deploy-time delta:** neutral (no infra/CDK/CI change; backend-only logic).
**Raised in:** `Task.Delay` audit, 2026-06-14 (site 4 of 5 — the only one flagged as a smell). **Actioned:** 2026-06-15.
**Depends on:** — (interacts with the 27-D command/query split).
...
**Deploy-time delta:** none (investigation only; the diagnostic is observer-only).
**Raised in:** TI-39 green-streak proof, 2026-06-16.
**Depends on:** likely the in-flight workspace-switcher/routing fixes.

---

## TI-43. Hard per-test E2E timeout so no single test can hang the deploy gate

✅ **Done** — 2026-06-17, PR #293, deploy #595.

**What:** added `E2EFactAttribute : FactAttribute { Timeout = 120_000 }` and replaced `[Fact]` with `[E2EFact]` across all 11 journey files (22 active + 1 skipped). Any single E2E test now hard-fails at **120 s** instead of being able to hang the suite indefinitely.

**Why:** PR #291's body-reading diagnostic hung the E2E suite **44 min** (deploy #592) — a single stuck Playwright call with no per-test bound stalls the whole gate until a human cancels. A hard cap makes that class of failure impossible regardless of cause.

**Why 120 s:** the reload-tolerant helpers each allow up to 30 s, and the Tags journeys chain 2–3 in one test → a legitimate worst case of ~90 s on a cold projector. 120 s never fails a healthy test while still capping a hang at 2 min.

**Verified, not assumed:** xUnit's `Timeout` is **silently ignored when parallelization is disabled**. Confirmed it fires in this assembly's config with a throwaway probe (`[Fact(Timeout=200)]` vs a 5 s delay → "Test execution timed out after 200 milliseconds"); probe removed after confirmation. The load-bearing precondition (no `xunit.runner.json`, no `[assembly: CollectionBehavior(DisableTestParallelization)]`) is documented in `E2EFactAttribute`.

**Deploy-time delta:** none (test-only).
**Raised in:** the 44-min hang investigation (TI-42 / BUG-31), 2026-06-17.
**Depends on:** —
