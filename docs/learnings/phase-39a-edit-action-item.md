...
A concurrent unrelated `TagsJourney` cold-start flake (app `tag-input` never loaded in 30 s; passed on rerun) co-failed the run. Both failures produced **only bare Playwright timeouts** — the journeys' `Console.WriteLine` browser logs are **swallowed by xUnit** and never reached the gh run log. Per the CLAUDE.md E2E guardrail, route evidence through the **thrown exception message**. Tracked as [BUG-38](../phases/phase-bugs-archive.md#bug-38) (fast-follow: add thrown-message diagnostics to the reload-tolerant assert helpers).

## 5. 39-B shipped through a shared red gate I didn't own — and my confident root-cause was wrong

39-B (the to-do mirror) was a clean, low-risk slice, but it sat un-mergeable for hours because a **parallel session's** `TodoReorderJourney` (37-A) deterministically red-gated `main` ([BUG-39](../phases/phase-bugs-archive.md#bug-39)). Two durable lessons:

1. **A shared red gate is yours to drive green, even another slice's failure** (codified in CLAUDE.md): I diagnosed it, couldn't fix it confidently (deployed-only, in-memory double masks it, not my feature), and took the sanctioned unblock — **quarantine-with-a-filed-bug** — to free every blocked session. Don't park and wait for the owner.
2. **Don't over-trust a static-reading root cause for a deployed-only bug.** I confidently blamed a per-item-`Position` fragility (`attribute_exists` swallow + `PutItem` clobber + cross-stream races) and recommended an order-snapshot redesign. The **real** cause, found by the 37-A session from *test-env projector logs*, was mundane and unrelated: `clear-test-data` didn't wipe `notetaker-proj-position`, so a **stable-id** stream (`todo-order#__default__`) carried a prior run's processed-position; the re-appended reorder (seq 1) was `≤` the stale mark and the projector skipped it as a duplicate (`position_guard`). Guid-keyed streams (`note#`/`todo#<guid>`) dodge it; stable-id streams collide across runs. The per-item fragilities were real but **latent**, not the trigger. Lesson: a deterministic deployed-only failure that no lower test reproduces is often **environment/state**, not logic — get the projector logs before betting on a redesign.
