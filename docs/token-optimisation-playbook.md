...
| Slice's own deploy failed **pre-`deploy-production`** (e.g. validate-frontend), then a **web/test-only** commit shipped the frontend without the backend → prod route-contract mismatch (404), burning a debug+roll-forward tail | A route-contract slice's FE (web asset) + BE (lambda+API GW routes) are on **independent deploy tracks**. After merging, confirm the **shipping** deploy had `backend=true` AND verify the prod route (`aws apigatewayv2 get-routes`). A pre-prod deploy failure leaves the BE undeployed; only a later `src/**` push carries it — `gh run rerun` replays the (web-only) sha and does NOT help | 34-B |
| Relocated a route prefix but left the **test doubles** on the old path → CI-only timing flake (msw miss → backoff retry firing after a `fetchSpy` → "no fetch" assertion breaks); local + PR CI passed | When moving a route prefix, grep+move its doubles in the **same** slice: frontend `GLOBAL_PATH_PREFIXES` + msw `scoped(`; backend `ScopedPrefixes`/`GlobalExceptions` in `ApiFactory` | 34-B |
