...
**Re-scoped by the 51-A spike (2026-08-10).** The phase opened as a redesign of the note's own Quick notes / Transcript / Final notes tabs; the human corrected it mid-spike — that strip is fine and is not changing, and the original problems about empty Transcript / Final notes tabs are **dropped, not deferred**. The real problem is the **open-note bar**: it exists only on a note screen, so going back to the notes list hides every note you had open and opening one makes the whole row reappear at once. 51-A prototyped four directions and locked **a pinned "My notes" tab**: the bar is present on every screen, the notes list is its permanent leftmost tab, nothing ever appears or disappears, and the line under the bar is replaced by merging the active tab into the page. **51-B** ships it; **51-C** (was 49-C) then marks the recording note in the same bar. One caveat carried forward with the design: the merge treatment repaints the main content area from `--color-bg` to `--color-surface` on *every* screen across 12 themes, so 51-B is an app-wide visual change needing a Stylist pass, not a one-component edit. Two E2E contracts also invert — `AssertNoOpenTabBarAsync` asserts the bar disappears, and the pinned tab must not share the `open-note-tab` testid or `CloseAllTabsExceptAsync` spins forever.
