...
Each note has an **agenda** — a checklist of topics the owner adds before/during a meeting and ticks off as covered, shown in the note **header** (expanded by default, collapsible to one "X / Y" line) and costing **no side space**. Five slices are live: **43-A** add, **43-B** tick/untick + coverage, **43-C** edit/remove, **43-D** collapsible strip + Stylist, **43-E** retired the legacy heading-✓ (BUG-37's "a topic = a heading" conflation). **A 2026-08-07 prototype + interview then re-cut the model**: because meeting notes are running prose rather than a section per topic, the heading-anchored tick could never fire — so the agenda moves *into* the body. Every task-list line in a note becomes a topic, the **body is canonical**, the tick is `- [x]` (a `ContentEdited`, not an agenda event), and the header strip becomes a view that writes back through the editor. Notably this needs **no identity token in the markdown** — the line *is* the item. **43-F** derived topics from the body (#428), **43-G** made the header write back through the editor (#438), and **43-H** closed the strangler in two halves: **43-H1** (#441) migrated the stragglers — 8 notes, 36 topics written into their bodies, applied 2026-08-10 and verified against the event stream (every original body preserved verbatim, +1 paragraph each, idempotent on re-run) — and only then **43-H2** (#451) dropped the legacy fold, the four write endpoints, the commands and the frontend hooks. The `AgendaItem*` events remain in the streams, unread and still parseable, which is what makes the removal reversible. Strangler-ordered throughout; deploy-time neutral. Splitting 43-H in two was the decision that mattered: migrating and removing in one deploy would have left those notes with no agenda between the deploy landing and the migration running. Detail: [docs/phases/phase-43.md](phases/phase-43.md).
...
Graduate the *expand the to-do functionality for today and the future* future-feature into a phase — **re-scoped on the way to contain no dates at all**. The original sketch assumed due/scheduled dates and calendar-derived Today/Upcoming/Overdue grouping; the user ruled that out (2026-08-06): today is *"an arbitrary line in the priority"*, drawn wherever they choose. So the phase adds a **user-positioned divider** to the existing ordered To Do list — items above it are Today, below are Later — persisted as an additive event on the existing `TodoOrdering` aggregate, with the grouping a frontend split of the already-ordered list. **50-A** the line exists, drags anywhere, and sticks across reloads; **50-B** a per-row `⋯` actions menu crosses the line in one click (and restores the keyboard reorder path CHANGE-29 removed). Pairs with [CHANGE-34], which ships independently on the same list. Three decisions moved during the build: the line is keyed **per user and per workspace**; a lost anchor is relocated **durably in the projector** rather than on read; and 50-B's review found `TodoOrderCommandHandler` was the only command handler with **no append retry** — on a stable-id stream where 50-B's paired reorder + line writes race each other into a 409 the client silently drops. Fixed with the standard bounded retry; see [phase-50b-two-writes-one-stream](learnings/phase-50b-two-writes-one-stream.md).
...
**Re-scoped by the 51-A spike (2026-08-10).** The phase opened as a redesign of the note's own Quick notes / Transcript / Final notes tabs; the human corrected it mid-spike — that strip is fine and is not changing, and the original problems about empty Transcript / Final notes tabs are **dropped, not deferred**. The real problem is the **open-note bar**: it exists only on a note screen, so going back to the notes list hides every note you had open and opening one makes the whole row reappear at once. 51-A prototyped four directions and locked **a pinned "My notes" tab**: the bar is present on every screen, the notes list is its permanent leftmost tab, nothing ever appears or disappears, and the line under the bar is replaced by merging the active tab into the page. **51-B shipped 2026-08-10** (#452, then #456 to fix the gate it broke; deploy #757, verified live) — the bar is on every screen with "My notes" pinned leftmost. **51-C** (was 49-C) then marks the recording note in the same bar. Six review rounds, and the E2E helper guarding the bar crashed the deploy gate on its first ever execution because E2E runs only in that gate — now runnable on demand per [TI-69]. See [phase-51b-open-note-bar.md](learnings/phase-51b-open-note-bar.md). The repaint it required turned out to be app-wide: every screen's content area moved from `--color-bg` to `--color-surface`, which measurably weakens plain `--color-border` outlines in the 9 dark themes (and strengthens them in the 8 light ones). 51-B added a `--color-border-strong` token across all 17 themes and applied it to the page-level controls that regressed; the rest are tracked as [CHANGE-39]. **51-C shipped 2026-09-11** (#468, deploy #776): a recording keeps running while you read another note, with the recording tab marked in the bar. Six review rounds, five spent on one misplaced side effect — see [phase-51c-recording-tab.md](learnings/phase-51c-recording-tab.md).
...
## Phase 52 — Find in a transcript _(In Progress — 52-A done 2026-09-08)_
...
**52-A shipped 2026-09-08** (#480, deploy #774). Three review rounds, eight findings, five of them invisible to every automated gate — including a version that highlighted nothing at all because a stray comment marker deleted the CSS rule while build, lint, both typechecks and 41 specs passed over it, and a fix that reset the user's position once per spoken phrase because it assumed a live transcript only ever grows. Three tests written to prove fixes turned out to pass with the defect present. Lessons and the four-command frontend gate: [phase-52a-tests-that-prove-nothing](learnings/phase-52a-tests-that-prove-nothing.md).
...
The [Summary table](phases/phase-bugs.md#summary) lists the **open** defects, one or two lines each in user terms, with the detail for each below it; every fixed bug is condensed into [phase-bugs-archive.md](phases/phase-bugs-archive.md), one entry per bug with its anchor preserved. A bug lives in exactly one of the two files — `scripts/check-doc-ids.sh` fails the commit if an id appears in both. This section deliberately does **not** re-list them, **and carries no fixed/open count** — the table is the single source. A hand-maintained total drifts exactly like the hand-maintained list did: this line read "49 bugs are fixed" on 2026-08-10 when the table held 67, having gone stale unnoticed across dozens of fixes. Count from the table when a number is needed; never cache one here.
