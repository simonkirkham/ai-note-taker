...
// per append and the consistency gate polls the same instance. A ConcurrentDictionary with the
// same monotonic semantics as the deployed DynamoDbProcessedPositionStore: an unseen stream
// reads -1, and SetLastSeqAsync never regresses. Not used in the API Lambda (no decorator
// there — the gate resolves DynamoDbProcessedPositionStore) nor the Projector Lambda (which
