...
// without re-running Google sign-in. Rotating: each use deletes the presented token and issues a new
// one. ExpiresAt caps its absolute lifetime — re-checked on use AND TTL-reaped — so a leaked token
