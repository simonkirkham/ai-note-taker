...
## BUG-88 — the live transcript recovers itself when the engine stops answering

**This is the one thing about BUG-88 that no test can prove.** The specs reproduce the state faithfully — a socket that accepts a connection and never answers — but the real failure was a *whisper-server* that hung from a healthy state and could not even be killed by the OS, and the end-to-end path has never been watched running in the packaged app. Until row 1 is ticked, the fix is reasoned, not observed.

Forcing the failure: start a recording in on-device mode, find the `whisper-server.exe` process, and **suspend** it (Process Explorer → right-click → Suspend, or `pssuspend`). A suspended process stays alive and holds its port but answers nothing — which is precisely the 2026-09-21 state. Resume or kill it afterwards.

| # | Check | ✓ |
|---|---|---|
| 1 | **A hang is recovered without anyone acting:** Given a recording running with words appearing, When I suspend `whisper-server.exe` for ~90 s and then leave it suspended, Then words stop for under a minute and **start appearing again by themselves**, and no failure banner appears. `local-transcription.log` contains `live engine replaced after it stopped answering`. | ☐ |
| 2 | **A new engine really is running:** Given row 1, Then the Windows process list shows a **second** `whisper-server.exe` with a later start time, on a different port. (The suspended one may be unkillable — that is expected and harmless.) | ☐ |
| 3 | **The next recording is not handed a corpse:** Given row 1's suspended engine still present, When I stop and start a NEW recording **without restarting the app**, Then the live transcript populates normally within ~10 s. Before this change it stayed empty and showed "stopped responding" after ~60 s. | ☐ |
| 4 | **Giving up is still possible:** Given I suspend each replacement as it appears, When the third engine is suspended, Then the banner says on-device transcription stopped responding, it appears **once**, and the app stops spawning engines. | ☐ |
