...
| **Transcript health** | Incomplete meeting transcripts do not alarm — this sweep is where they surface (TI-99). Logs Insights over the **Command** Lambda log group: `fields @timestamp, level, message, user_agent \| filter message like /^Transcript health/ \| filter level = "Warning" \| sort @timestamp desc \| limit 200`. Then `TranscriptCoverageRatio` (`NoteTaker/Domain`, `Service=note-taker`, Minimum) for any value under 0.8, and `TranscriptStalled` for any non-zero sum. How to read a line: [observability.md](../../../docs/observability.md#why-is-a-transcript-incomplete). Each incomplete transcript is a [BUG-85]-class finding — add the evidence there rather than filing a new bug unless the `end=` reason is new. |
