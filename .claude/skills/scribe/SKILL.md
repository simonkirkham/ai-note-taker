...
3. **Update progress tracking:**
   - `docs/phases/phase-N.md` — mark every completed acceptance criterion `[x]`, set slice status to `Done`
   - `docs/roadmap.md` — mark phase as `_(Done)_` if all slices are complete; update `_(In Progress)_` if partially done
...
5. **Commit** — single commit with message `docs: scribe notes for slice <id>`, covering all files from steps 1–4.
