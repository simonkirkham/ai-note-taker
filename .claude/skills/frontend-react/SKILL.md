...
- **A side effect owed once per event belongs to whatever outlives the component that shows it.** If keeping it exactly-once across a remount needs a latch, a claim or a "have I already…" flag held above the component, the effect is in the wrong place: move it into the owner of the event (a provider, the session) and let the component only display its progress. 51-C patched a shared "write-up claimed" flag across three review rounds — each fix broke another path, down to analysing an unrelated note silently cancelling a meeting's write-up — until moving the write-up into the recording session deleted the flag (PR #468).
