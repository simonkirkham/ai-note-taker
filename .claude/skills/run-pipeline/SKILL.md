...
2. *(gate if unspecced)* **Breaker** creates the worktree + branch (`slice/<phase>-<id>-<desc>`), `dotnet restore` + `npm --prefix web install`, `/rename` the session, writes the failing BDD spec.
...
8. Hawk requests changes → fix every finding, push, re-run Hawk. Hawk approves → check both merge gates, then merge.
...
- Anything that writes the same files in two concurrent worktrees without isolation.
